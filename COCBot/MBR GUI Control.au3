...
Func InitializeMainGUI($bGuiModeUpdate = False)
	...
	If $g_bDevMode Then
		...
		GUICtrlSetState($g_hChkDebugGetVillageSize, $GUI_SHOW + $GUI_ENABLE)
...
Func GUIControl_WM_COMMAND($hWind, $iMsg, $wParam, $lParam)
	...
	Switch $nID
		...
		Case $g_hPicRegisterNow
			; Donate URL is not in text nor tooltip
			ShellExecute("https://multibot.run/register")
		Case $g_hBtnStop
			btnStop()
		Case $g_hBtnPause
			btnPause()
		...
		Case $g_hBtnTestArmyWindow
			...
			$g_bRunState = True
			Setlog("Army Overview Test Start")
			_checkArmyCamp(False, False, False, True)
			Setlog("Army Overview Test End")
		...
		Case $g_hBtnTestAutoUpgrade
			;AutoUpgrade(True)
			Local $RuntimeA = $g_bRunState
			Local $Ocr = $g_bDebugOcr
			$g_bDebugOcr = True
			$g_bRunState = True
			$g_sLabUpgradeTime = ""
			$g_avLabTroops[$g_iCmbLaboratory][5] = 0
			Setlog("Lab Test starts")
			Laboratory(True)
			Setlog("Lab Test ends")
			$g_bRunState = $RuntimeA
			$g_bDebugOcr = $Ocr
			$g_sLabUpgradeTime = ""
			$g_avLabTroops[$g_iCmbLaboratory][5] = 0
		...
		Case $g_hBtnTestQuickTrainsimgloc
			...
			CheckQueueTroops()
			$g_bRunState = $RuntimeA
		Case $g_hBtnTestVillagePolygon
			btnTestVillagePolygon()
		Case $g_hBtnTestVillageRedlines
			DebugRedLines644()
		Case $g_hBtnTestWardenMode
			btnTestWardenMode()
...
Func BotGuiModeToggle()
	...
	Switch $g_iGuiMode
		...
		Case 1
			...
			GUICtrlDelete($g_hGUI_BUILDER_BASE_TAB)
...
Func SetRedrawBotWindow($bEnableRedraw, $bCheckRedrawBotWindow = Default, $bForceRedraw = Default, $RedrawControlIDs = Default, $sSource = "")
	...
	;SetDebugLog(($bEnableRedraw ? "Enable" : "Disable") & " MultiBot Window Redraw")
	...
	If $bEnableRedraw Then
		...
		EndIf
	Else
		SetDebugLog("Disable MultiBot Window Redraw" & (($sSource <> "") ? (": " & $sSource) : ("")))
...
Func CheckRedrawBotWindow($bForceRedraw = Default, $RedrawControlIDs = Default, $sSource = "")
	...
	If $g_bRedrawBotWindow[1] Or $bForceRedraw Then
		...
		If $RedrawControlIDs = Default Then
			...
			SetDebugLog("Redraw MultiBot Window" & ($bForceRedraw ? " (forced)" : "") & (($sSource <> "") ? (": " & $sSource) : (""))) ; enable logging to debug GUI redraw
			_WinAPI_RedrawWindow($g_hFrmBotEx, 0, 0, BitOR($RDW_INVALIDATE, $RDW_ALLCHILDREN, $RDW_ERASE))
		Else
			; redraw only specified control(s)
			If IsArray($RedrawControlIDs) Then
				SetDebugLog("Redraw MultiBot ControlIds" & ($bForceRedraw ? " (forced)" : "") & ": " & _ArrayToString($RedrawControlIDs, ", "))
				Local $c
				...
				Next
			Else
				SetDebugLog("Redraw MultiBot ControlId" & ($bForceRedraw ? " (forced)" : "") & ": " & $RedrawControlIDs)
...
Func RedrawBotWindowNow()
	SetDebugLog("Redraw MultiBot Window Now")
...
Func tabMain()
	...
	Select
		Case $tabidx = 0 ; Log
			...
			GUISetState(@SW_HIDE, $g_hGUI_DUMMY_GUIDE)
		...
		Case $tabidx = 1 ; Village
			...
			GUISetState(@SW_HIDE, $g_hGUI_BOT)
			GUISetState(@SW_HIDE, $g_hGUI_ABOUT)
			If $g_bChkPlayBBOnly Or $g_bChkFarmVersion Then
				setGuideTxt($tabidx)
				GUISetState(@SW_SHOWNOACTIVATE, $g_hGUI_DUMMY_GUIDE)
			Else
				GUISetState(@SW_HIDE, $g_hGUI_DUMMY_GUIDE)
...
Func setGuideTxt($tabidx)
	Local $tabName = _GUICtrlTab_GetItemText($g_hTabMain, $tabidx)
	Local $guideSubTxt = ""
	If $g_bChkFarmVersion Then
		$guideSubTxt = "Farm Version"
	ElseIf $g_bChkPlayBBOnly Then
		$guideSubTxt = "Play BB Only"
	EndIf
	Local $guideTxt = "NOTE: '" & $tabName & "' Tab is disabled If you wish to use this feature you have to Uncheck '" & $guideSubTxt & "'"
	GUICtrlSetData($g_hLblDummyTabGuide, $guideTxt)
EndFunc   ;==>setGuideTxt
...
Func Bind_ImageList($nCtrl, ByRef $hImageList)
	...
	Switch $nCtrl
		Case $g_hTabMain
			...
			Local $aIconIndex = [$eIcnHourGlass, $eIcnTH12, $eIcnAttack, $eIcnBuilderHall, $eIcnGUI, $eIcnInfo]
		...
		Case $g_hGUI_MISC_TAB
			Local $aIconIndex = [$eIcnTH1, $eIcnClanGames]
		...
		Case $g_hGUI_SEARCH_TAB
			...
			Local $aIconIndex = [$eIcnCollector, $eIcnCC, $eIcnOptions]
		...
		Case $g_hGUI_ATTACKOPTION_TAB
			...
			Local $aIconIndex = [$eIcnMagnifier, $eIcnCamp, $eIcnLightSpell, $eIcnTrophy]
		...
		Case $g_hGUI_STATS_TAB
			...
			Local $aIconIndex = [$eIcnGoldElixir, $eIcnCCRequest, $eIcnGoldElixir]
