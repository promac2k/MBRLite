...
MBR GUI Design.au3; CreateMainGUI()
   ...
   MBR GUI Design Guide.au3; CreateDummyGuideTab()
#ce
#Tidy_On
...
;~ ------------------------------------------------------
	...
	_WindowAppId($g_hFrmBot, "MultiBot.run")
...
Func CreateMainGUIControls($bGuiModeUpdate = False)
	...
	If Not $bGuiModeUpdate Then
		...
		If $g_bCustomTitleBarActive Then
			$g_hFrmBotButtons = GUICreate("MultiBot Title Buttons", $_GUI_MAIN_BUTTON_COUNT * $aBtnSize[0], $aBtnSize[1], $_GUI_MAIN_WIDTH - $aBtnSize[0] * $_GUI_MAIN_BUTTON_COUNT, 0, BitOR($WS_CHILD, $WS_TABSTOP), BitOR($WS_EX_TOOLWINDOW, $WS_EX_NOACTIVATE, ($g_bAndroidShieldPreWin8 ? 0 : $WS_EX_LAYERED)), $g_hFrmBot)
		...
		$g_hFrmBotEx = GUICreate("MultiBot Controls", $_GUI_MAIN_WIDTH, $_GUI_MAIN_HEIGHT - $_GUI_BOTTOM_HEIGHT + $_GUI_MAIN_TOP, 0, 0, _
		...
		Else
			...
			$g_hFrmBotLogoUrlSmall = GUICreate("MultiBot URL", 290, 13, 0, 0, BitOR($WS_CHILD, $WS_TABSTOP), BitOR($WS_EX_TOOLWINDOW, $WS_EX_NOACTIVATE, ($g_bAndroidShieldPreWin8 ? 0 : $WS_EX_LAYERED)), $g_hFrmBot)
		...
		$g_hLblAndroidinfos = GUICtrlCreateLabel($g_sAndroidEmulator & " v" & $g_sAndroidVersion, $_GUI_MAIN_WIDTH - 147, $_GUI_MAIN_TOP + 28, 130, 12, $SS_RIGHT)
		...
		$g_hFrmBotBottom = GUICreate("MultiBot Buttons", $_GUI_MAIN_WIDTH, $_GUI_BOTTOM_HEIGHT, 0, $_GUI_MAIN_HEIGHT - $_GUI_BOTTOM_HEIGHT + $_GUI_MAIN_TOP, _
				BitOR($WS_CHILD, $WS_TABSTOP), 0, $g_hFrmBot)
...
;~ ------------------------------------------------------
	...
	CreateDummyGuideTab();This GUI will be on those tab which are disabled
	CreateLogTab()
...
;~ ------------------------------------------------------
	...
	$g_hTabBuilderBase = GUICtrlCreateTabItem(GetTranslatedFileIni("MBR Main GUI", "Tab_04", "Builder Base"))
