...
Func ChkBBLab()
	If GUICtrlRead($g_hChkUpgradeTroops) = $GUI_CHECKED Then
		$g_bChkUpgradeTroops = True
		GUICtrlSetState($g_hLblBBNextUpgrade, $GUI_ENABLE)
		GUICtrlSetState($g_hCmbBBLaboratory, $GUI_ENABLE)
		GUICtrlSetState($g_hPicBBLabUpgrade, $GUI_SHOW)
		GUICtrlSetState($g_hChkUpgradeMachine, $GUI_ENABLE)
	Else
		$g_bChkUpgradeTroops = False
		$g_bChkUpgradeMachine = False
		GUICtrlSetState($g_hLblBBNextUpgrade, $GUI_DISABLE)
		GUICtrlSetState($g_hCmbBBLaboratory, $GUI_DISABLE)
		GUICtrlSetState($g_hPicBBLabUpgrade, $GUI_HIDE)
		GUICtrlSetState($g_hChkUpgradeMachine, $GUI_UNCHECKED)
		GUICtrlSetState($g_hChkUpgradeMachine, $GUI_DISABLE)
	EndIf
EndFunc
