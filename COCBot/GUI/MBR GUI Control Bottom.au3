...
Func Initiate()
	...
	If $g_hAndroidWindow <> 0 And ($g_bAndroidBackgroundLaunched = True Or AndroidControlAvailable()) Then
		SetLogCentered(" " & $g_sBotTitle & " Powered by MultiBot.run ", "~", $COLOR_DEBUG)
...
Func chkLiteVersion()
	...
	If $g_bChkFarmVersion Then
		If GUICtrlRead($g_hChkPlayBBOnly) = $GUI_CHECKED Then ; Uncheck BB
			_GUI_Value_STATE("UNCHECKED", $g_hChkPlayBBOnly)
			chkPlayBBOnly()
		EndIf
		; Design Village
...
Func ChkVerificationToken()
	_ChkVerificationToken(True)
EndFunc   ;==>ChkVerificationToken

Func _ChkVerificationToken($Reset = False)
	Local Static $tokenwasverified[2] = [False, False]
	Local Static $FirstRun = True

	If $Reset Then $FirstRun = True

	SetDebugLog("ChkVerificationToken started")
	SetDebugLog("Token Was Verified: " & _ArrayToString($tokenwasverified))
	SetDebugLog("FirstRun: " & $FirstRun)
	...
	If Not $tokenwasverified[0] Then
		GUICtrlSetColor($g_hBtnVerifyToken, $COLOR_RED)
		GUICtrlSetState($g_hTxtRegistrationToken, $GUI_ENABLE)
		GUICtrlSetState($g_hBtnVerifyToken, $GUI_ENABLE)
	Else
		GUICtrlSetColor($g_hBtnVerifyToken, $COLOR_GREEN)
		GUICtrlSetState($g_hTxtRegistrationToken, $GUI_DISABLE)
		GUICtrlSetState($g_hBtnVerifyToken, $GUI_DISABLE)
	EndIf
