...
Func chkQuickTrainCombo()
	If GUICtrlRead($g_ahChkArmy[0]) = $GUI_UNCHECKED And GUICtrlRead($g_ahChkArmy[1]) = $GUI_UNCHECKED And GUICtrlRead($g_ahChkArmy[2]) = $GUI_UNCHECKED Then
		...
		ToolTip(GetTranslatedFileIni("MBR GUI Design Child Attack - Troops", "ChkArmy_Info_01", "QuickTrainCombo: ")& @CRLF & _ 
		GetTranslatedFileIni("MBR GUI Design Child Attack - Troops", "ChkArmy_Info_02", "At least 1 Army Check is required! Default Army 1."))
		Sleep(2000)
		ToolTip('')
	EndIf
...
Func TotalSpellCountClick()
	...
	If $g_iTownHallLevel > 9 Or $g_iTownHallLevel = 0 Then
		...
		_GUI_Value_STATE("SHOW", $g_aiTrainArmySpellLevel[$eSpellBat] > 0 ? $groupBat : $groupIcnBat)
	EndIf
