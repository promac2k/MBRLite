...
Func LoadLanguagesComboBox()
	...
	While 1
		...
		$aLanguageFile[$iFileIndex][2] = _GUIImageList_AddIcon($hLangIcons, $g_sLibIconPath, Eval("e" & $aLanguageFile[$iFileIndex][0]) - 1)
...
Func btnTestDonateCC()
	...
	SetLog(_PadStringCenter(" Test DonateCC begin (" & $g_sLiteVersion & ")", 54, "="), $COLOR_INFO)
	$g_iDonationWindowY = 0
	Local $aDonWinOffColors[3][3] = [[0xFFFFFF, 0, 1], [0xFFFFFF, 0, 31], [0xABABA8, 0, 32]]
	Local $aDonationWindow = _MultiPixelSearch(409, 0, 410, $g_iGAME_HEIGHT, 1, 1, Hex(0xFFFFFF, 6), $aDonWinOffColors, 10)
...
Func btnTestRequestCC()
	...
	SetLog(_PadStringCenter(" Test RequestCC begin (" & $g_sLiteVersion & ")", 54, "="), $COLOR_INFO)
	RequestCC()
...
Func btnTestSendText()
	Local $currentRunState = $g_bRunState
	$g_bRunState = True
	SetLog(_PadStringCenter(" Test SendText begin (" & $g_sLiteVersion & ")", 54, "="), $COLOR_INFO)
...
Func btnTestAttackBar($bRemaining = False, $bIsScreenShot = False)
	Local $hAttackBarTimer = __TimerInit()
	...
	$g_bRunState = True
	If $bIsScreenShot Then
		Local $sImageFile = BeginImageTest() ;Get the attackbar remaining screenshot
		If @error <> 0 Then
			SetLog("Testing image cancelled, will use " & $g_sAndroidEmulator, $COLOR_INFO)
		Else
			SetLog("Testing image for remains: " & $bRemaining & " : " & $sImageFile, $COLOR_INFO)
		EndIf
	EndIf
	Setlog(_PadStringCenter(" Begin AttackBar Detection", 54, "="), $COlOR_INFO)

	; [0] = Troop Enum Cross Reference [1] = Slot position [2] = Quantities [3] = Slot Starting X
	Local $aAttackBar = StringSplit(AttackBarCheck($bRemaining, $LB, True), "|", $STR_NOCOUNT)
	Local $aTroop
	...
	If IsArray($aAttackBar) And UBound($aAttackBar, 1) >= 1 Then
		...
		For $i = 0 To UBound($aAttackBar, 1) - 1
			...
			If IsArray($aTroop) And UBound($aTroop) > 3 Then Setlog("- Slot " & $aTroop[1] & ": x" & $aTroop[2] & " " & GetTroopName($aTroop[0], $aTroop[2]) & " : Slot X » " & $aTroop[3], $COLOR_SUCCESS)
		Next
	EndIf
	Setlog("AttackBar Total Took Time For Detection And Ocr: " & Round(__TimerDiff($hAttackBarTimer), 2) & "ms", $COLOR_SUCCESS)
	Setlog(_PadStringCenter(" End AttackBar Detection ", 54, "="), $COlOR_INFO)

	If $bIsScreenShot Then EndImageTest() ; clear test image handle

	$g_bDebugOcr = $bCurrentOCR
	$g_bDebugImageSave = $bCurrentDebugImage
	$g_bRunState = $bCurrentOCR
EndFunc   ;==>btnTestAttackBar

Func btnTestAttackBarUI()
	If $g_hAttackbarUiForm <> 0 Then Return
	$g_hAttackbarUiForm = GUICreate("Test Attackbar", 374, 144, 192, 124)
	GUICtrlCreateLabel("Note: For Testing remaining troops slot you should first run 'Test Attackbar' Or 'Test Attackbar Via Screenshot' Otherwise Result Correct Result Not Guaranteed.", 8, 16, 353, 49)
	Local $btnTestAttackbarMain = GUICtrlCreateButton("Test Attackbar", 8, 72, 131, 25)
	Local $btnTestAttackbarRemains = GUICtrlCreateButton("Test Attackbar Remains", 8, 104, 131, 25)
	Local $btnTestAttackbarViaSS = GUICtrlCreateButton("Test Attackbar Via Screenshot", 152, 72, 211, 25)
	Local $btnTestAttackbarRemainsViaSS = GUICtrlCreateButton("Test Attackbar Remains Via Screenshot", 152, 104, 211, 25)
	GUISetState(@SW_SHOW)
	Local $iOpt = Opt("GUIOnEventMode", 0)
	Local $bRemaining = False, $bIsScreenShot = False
	While 1
		Switch GUIGetMsg()
			Case $GUI_EVENT_CLOSE
				ExitLoop
			Case $btnTestAttackbarMain
				ExitLoop
			Case $btnTestAttackbarRemains
				$bRemaining = True
				ExitLoop
			Case $btnTestAttackbarViaSS
				$bIsScreenShot = True
				ExitLoop
			Case $btnTestAttackbarRemainsViaSS
				$bIsScreenShot = True
				$bRemaining = True
				ExitLoop
		EndSwitch
	WEnd
	CloseAttackBarUI()
	btnTestAttackBar($bRemaining, $bIsScreenShot)
	Opt("GUIOnEventMode", $iOpt)
EndFunc   ;==>btnTestAttackBarUI

Func CloseAttackBarUI()
	If $g_hAttackbarUiForm = 0 Then Return
	GUIDelete($g_hAttackbarUiForm)
	$g_hAttackbarUiForm = 0
EndFunc   ;==>CloseAttackBarUI
...
Func btnTestAttackCSV()
	...
	ConvertInternalExternArea("btnTestAttackCSV")
...
Func btnTestGetLocationBuilding()
	...
	; reset village measures
	setVillageOffset(0, 0, 1)
	ConvertInternalExternArea("btnTestGetLocationBuilding")
	;SearchZoomOut($aCenterEnemyVillageClickDrag, True, "btnTestAttackCSV")
...
Func btnTestVillagePolygon()
	Local $currentRunState = $g_bRunState
	Setlog(" #### btnTestVillagePolygon ####")

	ZoomOut()
	Local $village = GetVillageSize()
	Setlog("GetVillageSize measured: " & $village[0] & ", Zoom factor: " & $village[1] & ", Offset: " & $village[2] & ", " & $village[3], $COLOR_INFO)

	ConvertInternalExternArea("btnTestVillagePolygon")
	DebugImage4VillagePolygon()

	Setlog(" #### btnTestVillagePolygon ####")
	$g_bRunState = $currentRunState
EndFunc   ;==>btnTestVillagePolygon

Func DebugRedLines644()
	Local $currentRunState = $g_bRunState
	Setlog(" #### Started ConvertInternalExternArea ####")
	...
	ZoomOut()
	_CaptureRegion2()
	ConvertInternalExternArea("DebugRedLines644")

	; The External and Internal Lines Debug Image
	Local $subDirectory = $g_sProfileTempDebugPath & "DebugRedLines644"
	...
	; $CocDiamondECD: 422,0|825,307|422,614|20,307
	Local $tempCocDiamondECD[4][2]
	For $i = 0 To UBound($aCocDiamondECDTemp) - 1
		Local $temp = StringSplit($aCocDiamondECDTemp[$i], ",", $STR_NOCOUNT)
		$tempCocDiamondECD[$i][0] = $temp[0]
		$tempCocDiamondECD[$i][1] = $temp[1]
	Next

	; $CocDiamondDCD: 422,38|775,307|422,576|70,307
	Local $tempCocDiamondDCD[4][2]
	For $i = 0 To UBound($aCocDiamondDCDTemp) - 1
		Local $temp = StringSplit($aCocDiamondDCDTemp[$i], ",", $STR_NOCOUNT)
		$tempCocDiamondDCD[$i][0] = $temp[0]
		$tempCocDiamondDCD[$i][1] = $temp[1]
	Next

	_GDIPlus_GraphicsDrawLine($hGraphic, $tempCocDiamondECD[0][0], $tempCocDiamondECD[0][1], $tempCocDiamondECD[1][0], $tempCocDiamondECD[1][1], $hPenYellow)
	_GDIPlus_GraphicsDrawLine($hGraphic, $tempCocDiamondECD[1][0], $tempCocDiamondECD[1][1], $tempCocDiamondECD[2][0], $tempCocDiamondECD[2][1], $hPenYellow)
	_GDIPlus_GraphicsDrawLine($hGraphic, $tempCocDiamondECD[2][0], $tempCocDiamondECD[2][1], $tempCocDiamondECD[3][0], $tempCocDiamondECD[3][1], $hPenYellow)
	_GDIPlus_GraphicsDrawLine($hGraphic, $tempCocDiamondECD[3][0], $tempCocDiamondECD[3][1], $tempCocDiamondECD[0][0], $tempCocDiamondECD[0][1], $hPenYellow)
	...
	Setlog(" #### Ended ConvertInternalExternArea ####")
	Setlog(" #### Started DebugRedLines644 Old Method ####")
	...
	If IsArray($result) Then
		...
		Local $aiPixelTopLeft = GetPixelSide($listPixelBySide, 1)
		Local $aiPixelBottomLeft = GetPixelSide($listPixelBySide, 2)
		Local $aiPixelBottomRight = GetPixelSide($listPixelBySide, 3)
		Local $aiPixelTopRight = GetPixelSide($listPixelBySide, 4)

		CleanRedArea($aiPixelTopLeft)
		CleanRedArea($aiPixelTopRight)
		CleanRedArea($aiPixelBottomLeft)
		CleanRedArea($aiPixelBottomRight)

		Setlog("RedArea cleaned")
		Setlog("	[" & UBound($aiPixelTopLeft) & "] pixels TopLeft")
		Setlog("	[" & UBound($aiPixelTopRight) & "] pixels TopRight")
		Setlog("	[" & UBound($aiPixelBottomLeft) & "] pixels BottomLeft")
		Setlog("	[" & UBound($aiPixelBottomRight) & "] pixels BottomRight")

		Setlog("Total of: " & UBound($aiPixelTopLeft) + UBound($aiPixelTopRight) + UBound($aiPixelBottomLeft) + UBound($aiPixelBottomRight))

		For $i = 0 To UBound($aiPixelTopLeft) - 1
			Local $point = $aiPixelTopLeft[$i]
			_GDIPlus_GraphicsDrawRect($hGraphic, $point[0] - 2, $point[1] - 2, 4, 4, $hPenRed)
		Next
		For $i = 0 To UBound($aiPixelTopRight) - 1
			Local $point = $aiPixelTopRight[$i]
			_GDIPlus_GraphicsDrawRect($hGraphic, $point[0] - 2, $point[1] - 2, 4, 4, $hPenRed)
		Next
		For $i = 0 To UBound($aiPixelBottomLeft) - 1
			Local $point = $aiPixelBottomLeft[$i]
			_GDIPlus_GraphicsDrawRect($hGraphic, $point[0] - 2, $point[1] - 2, 4, 4, $hPenRed)
		Next
		For $i = 0 To UBound($aiPixelBottomRight) - 1
			Local $point = $aiPixelBottomRight[$i]
			_GDIPlus_GraphicsDrawRect($hGraphic, $point[0] - 2, $point[1] - 2, 4, 4, $hPenRed)
		Next
	EndIf
	Setlog(" #### Ended DebugRedLines644 Old Method ####")
	Setlog(" #### Started DebugRedLines644 Imfloc Method ####")

	SearchRedLinesMultipleTimes($CocDiamondECD, 3, 500)
	...
	For $i = 0 To UBound($aiPixelTopLeft) - 1
		Local $point = $aiPixelTopLeft[$i]
		_GDIPlus_GraphicsDrawRect($hGraphic, $point[0] - 2, $point[1] - 2, 4, 4, $hPenBlue)
	Next
	For $i = 0 To UBound($aiPixelTopRight) - 1
		Local $point = $aiPixelTopRight[$i]
		_GDIPlus_GraphicsDrawRect($hGraphic, $point[0] - 2, $point[1] - 2, 4, 4, $hPenBlue)
	...
	Setlog(" #### Ended DebugRedLines644 Imfloc Method ####")

	$g_bRunState = $currentRunState
EndFunc   ;==>DebugRedLines644


Func DebugImage4VillagePolygon()
	...
	Setlog("$CocDiamondECD: " & $CocDiamondECD)
	Setlog("$CocDiamondECD: " & UBound($aCocDiamondECDTemp))

	Setlog("$CocDiamondDCD: " & $CocDiamondDCD)
	Setlog("$CocDiamondDCD: " & UBound($aCocDiamondDCDTemp))

	; $ExternalArea
	; $InternalArea

	If UBound($aCocDiamondECDTemp) < 1 And UBound($aCocDiamondDCDTemp) < 1 Then
		Setlog("Problem Getting the Points!!", $COLOR_ERROR)
		Return
	EndIf

	_CaptureRegion2()
	Local $subDirectory = $g_sProfileTempDebugPath & "NewIntExterAreas"
	DirCreate($subDirectory)
	Local $Date = @YEAR & "-" & @MON & "-" & @MDAY
	Local $Time = @HOUR & "." & @MIN & "." & @SEC
	Local $EditedImage = _GDIPlus_BitmapCreateFromHBITMAP($g_hHBitmap2)
	...
	Local $text = ""
	; ###### OFFSET WORK START #######
	Local $OffsetRef[2] = [202, 104]
	Local $offset2Use[2] = [0, 0]

	Local $sArea = 170 & "," & 75 & "|" & 270 & "," & 75 & "|" & 270 & "," & 240 & "|" & 170 & "," & 240
	Local $Offset = decodeSingleCoord(findImage("", @ScriptDir & "\imgxml\village\Offset\Offset_1_91.xml", $sArea, 1, False))
	If UBound($Offset) = 2 Then
		Setlog("$Offset detect: " & _ArrayToString($Offset), $COLOR_DEBUG)
		$offset2Use[0] = $Offset[0] - $OffsetRef[0]
		$offset2Use[1] = $Offset[1] - $OffsetRef[1]
		Setlog("Diff is x: " & $offset2Use[0] & " y: " & $offset2Use[1], $COLOR_DEBUG)
		_GDIPlus_GraphicsDrawRect($hGraphic, $Offset[0] - 5, $Offset[1] - 5, 10, 10, $hPenRed)
		_GDIPlus_GraphicsDrawRect($hGraphic, $OffsetRef[0] - 5, $OffsetRef[1] - 5, 10, 10, $hPenWhite)
		_GDIPlus_GraphicsDrawLine($hGraphic, $Offset[0], $Offset[1], $OffsetRef[0], $OffsetRef[1], $hPenYellow)
		$text &= "_Offset_" & $Offset[0] & "_" & $Offset[1] & "_"
	EndIf
	; ###### OFFSET WORK END #######

	For $i = 0 To UBound($ExternalArea) - 1
		Local $Cordinate = [$ExternalArea[$i][0], $ExternalArea[$i][1]]
		$Cordinate[0] += $offset2Use[0]
		$Cordinate[1] += $offset2Use[1]
		_GDIPlus_GraphicsDrawRect($hGraphic, $Cordinate[0] - 5, $Cordinate[1] - 5, 10, 10, $hPenRed)
	Next
	For $i = 0 To UBound($InternalArea) - 1
		Local $Cordinate = [$InternalArea[$i][0], $InternalArea[$i][1]]
		$Cordinate[0] += $offset2Use[0]
		$Cordinate[1] += $offset2Use[1]
		_GDIPlus_GraphicsDrawRect($hGraphic, $Cordinate[0] - 5, $Cordinate[1] - 5, 10, 10, $hPenBlue)
	Next
	...
	For $i = 0 To UBound($aCocDiamondECDTemp) - 1
		Local $temp = StringSplit($aCocDiamondECDTemp[$i], ",", $STR_NOCOUNT)
		$tempCocDiamondECD[$i][0] = $temp[0] + $offset2Use[0]
		$tempCocDiamondECD[$i][1] = $temp[1] + $offset2Use[1]
		;$tempCocDiamondECD[$i][0] = $temp[0]
		;$tempCocDiamondECD[$i][1] = $temp[1]
	Next
	...
	For $i = 0 To UBound($aCocDiamondDCDTemp) - 1
		Local $temp = StringSplit($aCocDiamondDCDTemp[$i], ",", $STR_NOCOUNT)
		$tempCocDiamondDCD[$i][0] = $temp[0] + $offset2Use[0]
		$tempCocDiamondDCD[$i][1] = $temp[1] + $offset2Use[1]
		;$tempCocDiamondDCD[$i][0] = $temp[0]
		;$tempCocDiamondDCD[$i][1] = $temp[1]
	Next
	...
	Local $filename = String($Date & "_" & $Time & $text & "_.png")
	_GDIPlus_ImageSaveToFile($EditedImage, $subDirectory & "\" & $filename)
	_GDIPlus_PenDispose($hPenRed)
	_GDIPlus_PenDispose($hPenWhite)
	_GDIPlus_PenDispose($hPenYellow)
	...
	_GDIPlus_GraphicsDispose($hGraphic)
	_GDIPlus_BitmapDispose($EditedImage)
EndFunc   ;==>DebugImage4VillagePolygon
