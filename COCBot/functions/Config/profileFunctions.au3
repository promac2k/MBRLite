...
Func aquireProfileMutex($sProfile = Default, $bReturnOnlyMutex = Default, $bShowMsgBox = False)
	...
	Local $hMutex_Profile = CreateMutex("MultiBot.run-Profile-" & $sProfile)
	If $bReturnOnlyMutex Then
		Return $hMutex_Profile
	EndIf

	Local $sMsg = StringRegExpReplace(GetTranslatedFileIni("MBR GUI Design - Loading", "Msg_Android_instance_02", "MultiBot with Profile %s is already in use.\r\n\r\n", $sProfile), "[\r\n]", "")
