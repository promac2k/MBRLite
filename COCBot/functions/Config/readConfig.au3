...
Func ReadProfileConfig($sIniFile = $g_sProfilePath & "\profile.ini")
	...
	$g_sTxtRegistrationToken = IniRead($sIniFile, "general", "TxtRegistrationToken", "")
...
Func ReadBuildingConfig()
	...
	Local $_ver200 = GetVersionNormalized("2.0.0")
	If $buildingVersion < $_ver200 Then
		SetLog("New Multibot.run version! Re-locate all buildings!", $COLOR_WARNING)
...
Func ReadConfig_Debug()
	...
	If $g_bDevMode Then
		...
		$g_bDebugGetVillageSize = IniRead($g_sProfileConfigPath, "debug", "DebugGetVillageSize", 0) = 1 ? True : False
	EndIf
...
Func ReadConfig_600_6()
	...
	IniReadS($g_bChkCTBoostAtkAvailable, $g_sProfileConfigPath, "other", "ChkCTBoostAtkAvailable", False, "Bool")
	IniReadS($g_bChkCleanYardBB, $g_sProfileConfigPath, "other", "chkCleanYardBB", False, "Bool")
	...
	IniReadS($g_bChkUpgradeTroops, $g_sProfileConfigPath, "other", "ChkUpgradeTroops", False, "Bool")
	IniReadS($g_bChkUpgradeMachine, $g_sProfileConfigPath, "other", "ChkUpgradeMachine", False, "Bool")

	IniReadS($g_iCmbBBLaboratory, $g_sProfileConfigPath, "other", "CmbBBLaboratory", 10, "int")
...
Func ReadConfig_600_12()
	...
	$g_asTxtDonateTroop[$eTroopIceGolem] = StringReplace(IniRead($g_sProfileConfigPath, "donate", "txtDonateIceGolems", "ice golem|ice"), "|", @CRLF)
	$g_asTxtBlacklistTroop[$eTroopIceGolem] = StringReplace(IniRead($g_sProfileConfigPath, "donate", "txtBlacklistIceGolems", "no ice golem|ice no"), "|", @CRLF)
	...
	$g_asTxtDonateTroop[$eTroopCount + $g_iCustomDonateConfigs + $eSiegeStoneSlammer] = StringReplace(IniRead($g_sProfileConfigPath, "donate", "txtDonateStoneS", "stone|slammer|siege"), "|", @CRLF)
	$g_asTxtBlacklistTroop[$eTroopCount + $g_iCustomDonateConfigs + $eSiegeStoneSlammer] = StringReplace(IniRead($g_sProfileConfigPath, "donate", "txtBlacklistStoneS", "no stone|no slammer|siege no"), "|", @CRLF)
