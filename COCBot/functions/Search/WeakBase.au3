...
Func weakBaseCheck($townHallLevel = 12, $redlines = "", $bForceCaptureRegion = True)
	; Setup default return coords of 0,0
	Local $defaultCoords[1][2] = [[0, 0]]
	; Setup Empty Results in case to avoid errors, levels are set to max level of each type
...
Func IsWeakBase($townHallLevel = 12, $redlines = "", $bForceCaptureRegion = True)
	...
	ElseIf $g_bDebugImageSave Then
		...
		Setlog("Debug option is set, so take a debug picture")
