...
;~ 	EndIf
	...
	While 1
		...
		If chkSearchText() = False Then
			If $g_bDebugSetlog Then SetDebugLog("Cloud Search Text not found. Let's Check If Base Found If Not Restart Search...", $COLOR_DEBUG)
			;When Cloud Text Not Found There is quite chance that cloud is gone and we found the base
			;So to be safe side check check if attack village found before returning False to restart search.
			If chkSurrenderBtn() = True Then Return True ; check if clouds are gone.
			If chkAttackSearchFail() = 1 Then Return True ; OCR text for search fail message, and press retry if available, success continue searching
			Return False
		Else
