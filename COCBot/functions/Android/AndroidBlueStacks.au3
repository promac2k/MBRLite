...
Func OpenBlueStacks2($bRestart = False)
	...
	While $g_hAndroidControl = 0
		...
		Else
			$PID = ProcessExists2($g_sAndroidProgramPath, $g_sAndroidInstance)
		EndIf
...
Func CloseUnsupportedBlueStacksX($bClose = True)
	...
	If IsArray(ControlGetPos("Bluestacks App Player", "", "")) Or ($sPartnerExePath And ProcessExists2($sPartnerExePath)) Then ; $g_avAndroidAppConfig[1][4]
		...
		If $bClose = True Then
			SetLog("MultiBot doesn't work with " & $g_sAndroidEmulator & " App Player", $COLOR_ERROR)
			SetLog("Please let MultiBot start " & $g_sAndroidEmulator & " automatically", $COLOR_INFO)
