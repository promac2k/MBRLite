...
Global $g_sAndroidAdbPrompt = "multibot.run:" ; Unique ADB PS1 prompt
...
Func UpdateAndroidConfig($instance = Default, $emulator = Default)
	...
	Else
		$g_sAndroidPicturesHostFolder = "multibot.run\"
	EndIf
...
Func UpdateHWnD($hWin, $bRestart = True)
	...
	#cs Was coded for LeapDroid when manually launch DPI unaware and bot was DPI aware, then bot resize LeapDroid but too pixy... unfortunately making bot DPI aware did not fix it, so better just restart bot?
		...
		EndIf
	#ce Was coded for LeapDroid when manually launch DPI unaware and bot was DPI aware, then bot resize LeapDroid but too pixy... unfortunately making bot DPI aware did not fix it, so better just restart bot?
	GetAndroidControlClass(True, True)
	...
	If $runningInstance = "" And @error <> 0 Then ; Not implemented
		...
		If UBound($pids) > 0 Then
			...
			For $i = 0 To UBound($pids) - 1
				...
				SetDebugLog("GetAndroidRunningInstance: Found " & DoubleQuote($commandLine) & " by PID=" & $pid)
				...
				If WinGetAndroidHandle() <> 0 Then
					SetDebugLog("Running " & $g_sAndroidEmulator & " instance found: " & DoubleQuote($g_sAndroidInstance))
			...
			Next
		EndIf
		If $a[0] <> 0 Then SetDebugLog("Running " & $g_sAndroidEmulator & " instance is " & DoubleQuote($g_sAndroidInstance))
		Return FuncReturn($a)
	EndIf
...
Func _OpenAndroid($bRestart = False, $bStartOnlyAndroid = False)
	...
	If Not InitAndroid() Then
		...
		SetLog("To switch to another Emulator, please use bot with command line parameter", $COLOR_BLUE)
...
Func WaitForRunningVMS($WaitInSec = 120, $hTimer = 0)
	...
	While True
		...
		If Not $g_bRunState Then Return True
		$running = StringInStr($cmdOutput, DoubleQuote($g_sAndroidInstance)) > 0
...
Func AquireAdbDaemonMutex($timout = 30000)
	...
	While $g_hMutex_MyBot = 0 And __TimerDiff($timer) < $timout
		$g_hMutex_MyBot = CreateMutex("MultiBot.run/AdbDaemonLaunch")
	...
	Switch $g_iAndroidRecoverStrategy
		...
		Case 1
			...
			If $rebootAndroidIfNeccessary Then
				...
				If Not $bRebooted Then Return 0
			Else
				SetDebugLog("ConnectAndroidAdb: Reboot Android or ADB Daemon not allowed", $COLOR_ERROR)
				Return 0
			EndIf
...
Func RebootAndroid($bRestart = True, $bStartOnlyAndroid = False)
	...
	; Just for developer purposes!
	$g_iEmulatorRebots += 1
	If FileExists(@ScriptDir & "\EnableMBRDebug.txt") Then
		GUICtrlSetData($g_lblEmulatorReboots, "Emulator Reboots: " & $g_iEmulatorRebots)
		GUICtrlSetState($g_lblEmulatorReboots, $GUI_SHOW)
	EndIf

	; Small delay until open the emulator , for Slow PC's
	If _Sleep(15000) Then Return FuncReturn(False)
...
Func CheckScreenAndroid($ClientWidth, $ClientHeight, $bSetLog = True)
	...
	If Not $ok Then
		If $bSetLog Then SetLog("MultiBot doesn't work with " & $g_sAndroidEmulator & " screen resolution of " & $ClientWidth & " x " & $ClientHeight & "!", $COLOR_ERROR)
	...
	If $font_scale > 0 Then
		...
		If $font_scale <> 1 Then
			SetLog("MultiBot doesn't work with Display Font Scale of " & $font_scale, $COLOR_ERROR)
			Return False
		EndIf
	Else
	...
	If Not $g_bAndroidInitialized Or $g_iAndroidAdbProcess[0] = 0 Or ProcessExists2($g_iAndroidAdbProcess[0]) <> $g_iAndroidAdbProcess[0] Then
		...
		If True Then ; always validate picture patch
			...
			If $pathFound = False Then
				SetLog($g_sAndroidEmulator & " cannot use ADB on shared folder, " & DoubleQuote($g_sAndroidPicturesPath) & " not found", $COLOR_ERROR)
			EndIf
			; clear output
		...
		If StringLen($g_sAndroidMouseDevice) > 0 And $g_sAndroidMouseDevice = $g_avAndroidAppConfig[$g_iAndroidConfig][13] Then
			...
			If StringInStr($g_sAndroidMouseDevice, "/dev/input/event") = 0 Then
				...
				Local $aRegExResult = StringRegExp($s, "(\/dev\/input\/event\d+)[\r\n]+.+" & Chr(34) & $g_sAndroidMouseDevice & Chr(34) & "((?!\/dev\/input\/event)[\s\S])+ABS", $STR_REGEXPARRAYMATCH)
				If @error = 0 Then
				...
				Else
					...
					$aRegExResult = StringRegExp($s, "(\/dev\/input\/event\d+)[\r\n]+.+" & Chr(34) & ".+" & Chr(34) & "((?!\/dev\/input\/event)[\s\S])+ABS \(\d+\): 0035.+max " & $g_iGAME_WIDTH & ".+\n.+0036.+max " & $g_iGAME_HEIGHT, $STR_REGEXPARRAYMATCH)
					If @error = 0 Then
						SetDebugLog("Using " & $aRegExResult[0] & " for mouse events (" & $g_sAndroidMouseDevice & " not found)")
						$g_sAndroidMouseDevice = $aRegExResult[0]
					Else
						$g_bAndroidAdbClick = False
						SetLog($g_sAndroidEmulator & " cannot use ADB for mouse events, " & DoubleQuote($g_sAndroidMouseDevice) & " not found", $COLOR_ERROR)
						Return SetError(2, 1)
					EndIf
				EndIf
			EndIf
...
Func _AndroidAdbSendShellCommand($cmd = Default, $timeout = Default, $wasRunState = Default, $EnsureShellInstance = True, $bStripPrompt = True, $bNoShellTerminate = False)
	...
	If $error <> 0 Then
		...
		setDebugLog("ADB shell PID = : " & $g_iAndroidAdbProcess[0])
		$iCommandErrors += 1
	EndIf
...
Func AndroidAdbSendShellCommandScript($scriptFile, $variablesArray = Default, $combine = Default, $timeout = Default, $wasRunState = $g_bRunState, $EnsureShellInstance = True)
	...
	Else
		...
		EndIf
		$s = AndroidAdbSendShellCommand("sh " & DoubleQuote($androidPath & $scriptFileSh), $timeout, $wasRunState, $EnsureShellInstance)
...
Func _AndroidScreencap($iLeft, $iTop, $iWidth, $iHeight, $iRetryCount = 0)
	...
	$s = AndroidAdbSendShellCommand("screencap " & DoubleQuote($androidPath & $Filename), $g_iAndroidAdbScreencapWaitAdbTimeout, $wasRunState)
	;$s = AndroidAdbSendShellCommand("screencap " & DoubleQuote($androidPath & $filename), -1, $wasRunState)
...
Func AndroidZoomOut($overWaters = False, $loopCount = 0, $timeout = Default, $wasRunState = $g_bRunState)
	Return BuilderBaseSendZoomOut(True)
...
Func AndroidMoveMouseAnywhere()
	...
	$g_bSilentSetLog = True
	AndroidAdbSendShellCommand("dd if=" & DoubleQuote($androidPath & $Filename) & " of=" & $g_sAndroidMouseDevice & " obs=" & $iToWrite & ">/dev/null 2>&1", Default)
	If BitAND($g_iAndroidSecureFlags, 2) = 2 Then
		; delete file
		FileDelete($hostPath & $Filename)
	EndIf
...
Func AndroidSendText($sText, $SymbolFix = False, $wasRunState = $g_bRunState)
	...
	If $error = 0 Then
		...
		Local $newText = $sText
		StringRegExpReplace($newText, "[^A-Za-z0-9\.,\?" & Chr(34) & "!@#\$%\^&\*\(\)-_=\+;:<>\/\\\|\}\{\[\]'~ ]", ".")
		If @extended <> 0 Then
		...
		If $SymbolFix = False Then
			If $g_iAndroidAdbInputWordsCharLimit = 0 Then
				...
				$newText = StringRegExpReplace($newText, "([\\\?" & Chr(34) & "\$\^&\*\(\)\+<>\|'~;])", "\\$1")
				; replace " " with "%s"
			...
			Else
				...
				For $i = 1 To $words[0]
					...
					While StringLen($word) > 0
						; escape special characters
						$newWord = StringRegExpReplace(StringLeft($word, $g_iAndroidAdbInputWordsCharLimit), "([\\\?" & Chr(34) & "\$\^&\*\(\)\+<>\|'~;])", "\\$1")
...
Func GetAndroidProcessPID($sPackage = Default, $bForeground = True, $iRetryCount = 0)
	...
	Local $cmd = "set result=$(ps -p|grep " & DoubleQuote($g_sAndroidGamePackage) & " >&2)"
...
Func OpenAdbShell($bRunInitScript = True)
	...
	SetLog("Launched ADB Shell, PID=" & $iPID & ": " & DoubleQuote($g_sAndroidAdbPath) & " -s " & $g_sAndroidAdbDevice & " shell")
	Local $hWnd = 0
	Local $hTimer = __TimerInit()
	Do
...
Func PullSharedPrefs($sProfile = $g_sProfileCurrentName)
	...
	If $g_bPullPushSharedPrefsAbdCommand Then
		$cmdOutput = LaunchConsole($g_sAndroidAdbPath, "-s " & $g_sAndroidAdbDevice & " pull /data/data/" & $g_sAndroidGamePackage & "/shared_prefs " & DoubleQuote($g_sPrivateProfilePath & "\" & $sProfile & "\shared_prefs"), $process_killed)
	Else
		$cmdOutput = ""
	EndIf
...
Func PushSharedPrefs($sProfile = $g_sProfileCurrentName, $bCloseGameIfRunning = True)
	...
	If $g_bPullPushSharedPrefsAbdCommand Then
		$cmdOutput = LaunchConsole($g_sAndroidAdbPath, "-s " & $g_sAndroidAdbDevice & " push " & DoubleQuote($g_sPrivateProfilePath & "\" & $sProfile & "\shared_prefs") & " /data/data/" & $g_sAndroidGamePackage & "/shared_prefs", $process_killed)
	Else
		$cmdOutput = ""
	EndIf
...
Func CheckEmuNewVersions()
	...
	Local $NewVersion = ""
	Local $HelpLink = "Please visit Multibot Forum!"

	Switch $g_sAndroidEmulator
		Case "BlueStacks2"
			$NewVersion = GetVersionNormalized("4.3.29.0") ; 4.3.28.4020 tested
		Case "MEmu"
			$NewVersion = GetVersionNormalized("5.2.3.0")
		Case "Nox"
			$NewVersion = GetVersionNormalized("6.2.7.1")
		Case Else
			$NewVersion = GetVersionNormalized("25.1.0.0")
	EndSwitch
