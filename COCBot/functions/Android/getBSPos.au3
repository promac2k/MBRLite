...
Func getBSPos()
	...
	If @error = 1 Then
		...
		If @error = 1 Then
			...
			Local $stext = @CRLF & GetTranslatedFileIni("MBR Popups", "sText_01", "MultiBot has experienced a serious error") & @CRLF & @CRLF & _
					...
					GetTranslatedFileIni("MBR Popups", "sText_04", "and search www.multibot.run forums for more help") & @CRLF
			Local $MsgBox = _ExtMsgBox(0, GetTranslatedFileIni("MBR Popups", "sText_05", "Close MultiBot!"), GetTranslatedFileIni("MBR Popups", "sText_06", "Okay - Must Exit Program"), $stext, 15, $g_hFrmBot)
			If $MsgBox = 1 Then
				BotClose()
			EndIf
		EndIf
	...
	If Not IsArray($aPos) Then
		...
		If @error = 1 Then
			_ExtMsgBoxSet(1 + 64, $SS_CENTER, 0x004080, 0xFFFF00, 12, "Comic Sans MS", 600)
			$stext = @CRLF & GetTranslatedFileIni("MBR Popups", "sText_01", "MultiBot has experienced a serious error") & @CRLF & @CRLF & _
					GetTranslatedFileIni("MBR Popups", "sText_02", "Unable to find or start up") & " " & $g_sAndroidEmulator & @CRLF & @CRLF & GetTranslatedFileIni("MBR Popups", "sText_03", "Reboot PC and try again,") & _
					GetTranslatedFileIni("MBR Popups", "sText_04", "and search www.multibot.run forums for more help") & @CRLF
			$MsgBox = _ExtMsgBox(0, GetTranslatedFileIni("MBR Popups", "sText_05", "Close MultiBot!"), GetTranslatedFileIni("MBR Popups", "sText_06", "Okay - Must Exit Program"), $stext, 15, $g_hFrmBot)
			If $MsgBox = 1 Then
				BotClose()
				Return
			EndIf
...
Func getAndroidPos($FastCheck = False, $RetryCount1 = 0, $RetryCount2 = 0, $bWidthFirst = Default)
		...
		If $aControlSize[2] <> $g_iAndroidClientWidth Or $aControlSize[3] <> $g_iAndroidClientHeight Then ; Is Client size correct?
			...
			Switch $RetryCount1
				...
				Case 3
					...
					EndIf
				Case 4
					; it can happen that in hires DPI the Android Control cannot be resized to 860x644, so make Android DPI aware
	...
	If $bResizedOK Then
		;RedrawAndroidWindow()
		If $g_bDebugSetlog Then
			SetDebugLog($sPre & $g_sAndroidEmulator & " window resized to work with MultiBot", $COLOR_SUCCESS)
		Else
			SetLog($g_sAndroidEmulator & " window resized to work with MultiBot", $COLOR_SUCCESS)
		EndIf
		#cs
		Local $hCtrl = ControlGetHandle($g_hAndroidWindow, $g_sAppPaneName, $g_sAppClassInstance)
		Local $hCtrlTarget = __WinAPI_GetParent($hCtrl)
		...
		If $hCtrl And $hCtrlTarget And UBound($aCtrlTargetPos) > 2 Then
			...
			SetLog($sPre & $g_sAndroidEmulator & " window resized to work with MultiBot", $COLOR_SUCCESS)
		Else
