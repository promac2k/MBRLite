...
Func CheckVersion()
	...
	Local $sCorrectStdOut = InetRead("https://api.github.com/repos/promac2k/MBRLite/releases/latest")
	If @error Or $sCorrectStdOut = "" Then Return
	Local $Temp = BinaryToString($sCorrectStdOut)

	If $Temp <> "" And Not @error Then
		Local $g_aBotVersionN = StringSplit($g_sLiteVersion, " ", 2)
		If @error Then
			Local $g_iBotVersionN = StringReplace($g_sLiteVersion, "v", "")
		Else
			Local $g_iBotVersionN = StringReplace($g_aBotVersionN[0], "v", "")
		EndIf
		Local $version = GetLastVersion($Temp)
		$g_sBotGitVersion = StringReplace($version[0], "MBRLite_v", "")
		...
		ElseIf _VersionCompare($g_iBotVersionN, $g_sBotGitVersion) = 0 Then
			SetLog("WELCOME CHIEF, YOU HAVE THE LATEST MULTIBOTLITE VERSION", $COLOR_SUCCESS)
		Else
			SetLog("YOU ARE USING A FUTURE MULTIBOTLITE VERSION CHIEF!", $COLOR_ACTION)
		EndIf
		Return True
	Else
		SetDebugLog($Temp)
