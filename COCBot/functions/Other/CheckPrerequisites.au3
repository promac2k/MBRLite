...
Func isEveryFileInstalled($bSilent = False)
	...
	If @Compiled Then ;if .exe
		If Not StringInStr(@ScriptFullPath, "MultiBot.run.exe", 1) Then ; if filename isn't MyBot.run.exe
			If Not $bSilent Then
				...
				$sText2 = GetTranslatedFileIni("MBR Popups", "CheckPrerequisites_Item_05", 'You have renamed the file "MultiBot.run.exe"! Please change it back to MultiBot.run.exe and restart the bot!')
				$sText3 = GetTranslatedFileIni("MBR Popups", "CheckPrerequisites_Item_03", "Sorry, Start button disabled until fixed!")
...
Func checkIsAdmin($bSilent = False)
	...
	If Not $bSilent Then SetLog("MultiBot running without admin privileges", $COLOR_ERROR)
	Return False
