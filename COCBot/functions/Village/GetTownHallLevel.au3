...
Func VerifyAndClickTownHall($bCloseTownhallInfo = False)
	Local $aTHInfo[3] = ["", "", ""]
	Local $bIsTownhallFound = False
	If $g_bDebugSetlog Then SetDebugLog("Town Hall Position: " & $g_aiTownHallPos[0] & ", " & $g_aiTownHallPos[1], $COLOR_DEBUG)
	If Not isInsideDiamond($g_aiTownHallPos) Then ; If TH pos is not known or is outside village then get new position
		LocateTownHall(True) ; Set flag = true for location only, or repeated loop happens
		If isInsideDiamond($g_aiTownHallPos) Then SaveConfig() ; save new location
		If _Sleep($DELAYGETTHLEVEL1) Then Return
	EndIf
	ClickP($aAway, 1, 0, "#0350") ; Click away
	...
	If $aTHInfo[0] > 1 Then
		If StringInStr($aTHInfo[1], "Town") = 0 Then
			SetLog("Town Hall not found! I detected a " & $aTHInfo[1] & "! Please locate again!", $COLOR_WARNING)
			Return $bIsTownhallFound
		EndIf
		If $aTHInfo[2] <> "" Then
			SetLog("Your Town Hall Level read as: " & $aTHInfo[2], $COLOR_SUCCESS)
			$bIsTownhallFound = True
			If $bCloseTownhallInfo Then ClickP($aAway, 1, 0, "#0351") ; Unselect TH
		Else
			SetLog("Your Town Hall Level was not found! Please Manually Locate", $COLOR_INFO)
			ClickP($aAway, 1, 0, "#0352") ; Unselect TH
		EndIf
	Else
		SetLog("Your Town Hall Level was not found! Please Manually Locate", $COLOR_INFO)
		ClickP($aAway, 1, 0, "#0353") ; Unselect TH
	EndIf
	Return $bIsTownhallFound
EndFunc   ;==>VerifyAndClickTownHall
