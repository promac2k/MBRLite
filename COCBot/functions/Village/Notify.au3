...
Func NotifyReport()
	...
	If $g_bNotifyAlertVillageReport = True Then
		...
		$text &= " [" & GetTranslatedFileIni("MBR Func_Notify", "StatsBB-T_Info_01", "T") & "]: " & $g_aiCurrentLootBB[$eLootTrophyBB] & "%0A"
		$text &= " [" & GetTranslatedFileIni("MBR Func_Notify", "Free-BBBuilders_Info_01", "No. of Free Builders") & "]: " & $g_iFreeBuilderCountBB
...
Func NotifyPushFileToTelegram($File, $Folder, $FileType, $body)
	...
	If $g_bNotifyTGEnable And $g_sNotifyTGToken <> "" Then
		...
		Else
			...
			NotifyPushToTelegram($g_sNotifyOrigin & " | " & GetTranslatedFileIni("MBR Func_Notify", "Uploading-File_Info_03", "Unable to Upload File") & "%0A" & GetTranslatedFileIni("MBR Func_Notify", "Uploading-File_Info_02", "Occurred an error type 2 uploading file to Telegram server..."))
		EndIf
	EndIf
	...
	If $g_bNotifyTGEnable And $g_sNotifyTGToken <> "" Then
		...
		If $g_iTGLastRemote <> $g_sTGLast_UID Then
			...
			Switch $TGActionMSG
				...
				Case Else
					NotifyPushToTelegram($g_sNotifyOrigin & " | " & GetTranslatedFileIni("MBR Func_Notify", "Request-Start_Info_03", "Start MultiBot first."))
			EndSwitch
		EndIf
	EndIf
...
Func NotifyPushMessageToBoth($Message, $Source = "")
	...
	Switch $Message
		...
		Case "BuilderInfo"
			...
			; Check if you are at Main Village
