...
Func SwitchCOCAcc($NextAccount)
	...
	If $bResult = True Then
		...
		If $g_sProfileCurrentName <> $g_asProfileName[$g_iNextAccount] Then
			BuilderBaseResetStats() ; just to clean UI values
...
Func SwitchCOCAcc_ClickAccount(ByRef $bResult, $NextAccount, $bStayDisconnected = $g_bChkSharedPrefs, $bLateDisconnectButtonCheck = True)
	...
	For $i = 0 To 20 ; Checking Account List continuously in 20sec
		If _ColorCheck(_GetPixelColor($aListAccount[0], $aListAccount[1], True), Hex($aListAccount[2], 6), $aListAccount[3]) Then ;	Grey
			...
			Local $XCoordinates = QuickMIS("CX", $sGPlayAccount, 155, 100, 705, 620, True, $g_bDebugImageSave) ; RC Done
...
Func SwitchCOCAcc_ConfirmSCID(ByRef $bResult)
	For $x = 0 To 20 ; Checking LogOut & Confirm button continuously in 20sec
		...
		Local $AlternativeToMemuLogOut = [$aButtonLogOutSCID[0], 200, $aButtonLogOutSCID[2], $aButtonLogOutSCID[3]] ; RC Done
		Local $AlternativeToMemuConfirm = [410, 429 + $g_iMidOffsetYNew, $aButtonConfirmSCID[2], $aButtonConfirmSCID[3]] ; RC Done
		Local $aToCheckLogOut[2] = [$AlternativeToMemuLogOut, $aButtonLogOutSCID]
		Local $aToCheckConfirm[2] = [$AlternativeToMemuConfirm, $aButtonConfirmSCID]
...
Func SwitchCOCAcc_ClickAccountSCID(ByRef $bResult, $NextAccount, $iStep = 4)
	Local $YCoord = Int(292 + 73.5 * $NextAccount) ; RC Done
	...
	For $i = 0 To 30 ; Checking "Log in with SuperCell ID" button continuously in 30sec
		...
		If QuickMIS("BC1", $g_sImgLoginWithSupercellID, 260, 580, 435, 610, True, $g_bDebugImageSave) Then

			; SC ID selection found
			SetLog("Verified Log in with Supercell ID boot screen")
			Click($g_iQuickMISWOffSetX, $g_iQuickMISWOffSetY, 1, 0, "Click Log in with SC_ID")
			If _Sleep(2000) Then Return

			For $j = 0 To 20 ; Checking Account List continuously in 20sec
				Local $Window = QuickMIS("CX", $g_sImgSCIDWindow, 450, 120, 570, 240, True, $g_bDebugImageSave) ; RC Done
				If UBound($Window) > 0 Then
					Local $YLastCoordinate = 0
					For $i = 0 To 10
						Local $XCoordinates = QuickMIS("CX", $g_sImgSCID, 570, 120, 680, 550, True, $g_bDebugImageSave) ; RC Done
						If UBound($XCoordinates) > 0 Then
							SetDebugLog("[SCID Accounts]: " & UBound($XCoordinates), $COLOR_DEBUG)
							For $j = 0 To UBound($XCoordinates) - 1
								Local $Coordinates = StringSplit($XCoordinates[$j], ",", 2)
								; Let's check duplicated coordinates
								SetDebugLog("Coordinate Y: " & Number($Coordinates[1]) + 121)
								SetDebugLog("Last Y: " & $YLastCoordinate)
								If (Number($Coordinates[1]) + 121 > $YLastCoordinate + 25 Or Number($Coordinates[1]) + 121 < $YLastCoordinate - 25) Then
									Local $Duplicated = False
									For $x = 0 To UBound($AccountsCoord) - 1
										If $AccountsCoord[$x][1] < Number($Coordinates[1]) + 121 + 25 And $AccountsCoord[$x][1] > Number($Coordinates[1]) + 121 - 25 Then $Duplicated = True
									Next
									If Not $Duplicated Then
										ReDim $AccountsCoord[UBound($AccountsCoord) + 1][2]
										$AccountsCoord[UBound($AccountsCoord) - 1][0] = $Coordinates[0] + 550
										$AccountsCoord[UBound($AccountsCoord) - 1][1] = $Coordinates[1] + 121
										$YLastCoordinate = $Coordinates[1] + 121
									EndIf
								EndIf
							Next
							_ArraySort($AccountsCoord, 0, 0, 0, 1) ; short by column 1 [Y]
							...
							If $NextAccount + 1 > UBound($XCoordinates) Then
								...
								ExitLoop
							EndIf
							Click($AccountsCoord[$NextAccount][0] - 250, $AccountsCoord[$NextAccount][1], 1)
							SetLog("Please wait for loading CoC...!")
							$bResult = True
							Return "OK"
						EndIf
					...
					Return "Error"
				EndIf
				SetDebugLog("Waiting for SC ID Window!", $COLOR_ACTION)
				If $j = 20 Then
					$bResult = False
					Return "Error"
				EndIf
		...
		SetDebugLog("Checking 'Log in with SuperCell ID' button at '260,580(125,30)'", $COLOR_DEBUG)
		If $i = 30 Then
			$bResult = False
			Return "Error"
		EndIf
...
Func aquireSwitchAccountMutex($iSwitchAccountGroup = $g_iCmbSwitchAcc, $bReturnOnlyMutex = False, $bShowMsgBox = False)
	Local $sMsg = GetTranslatedFileIni("MBR GUI Design Child Bot - Profiles", "Msg_SwitchAccounts_InUse", "MultiBot with Switch Accounts Group %s is already in use or active.", $iSwitchAccountGroup)
...
Func CheckGoogleSelectAccount($bSelectFirst = True)
	...
	If _ColorCheck($pColor, Hex($aListAccount[2], 6), $aListAccount[3]) Then ; White
		...
		; Account List check be there, validate with imgloc
		If UBound(decodeSingleCoord(FindImageInPlace("GoogleSelectAccount", $g_sImgGoogleSelectAccount, "180,355(90,300)", False))) > 1 Then ; RC Done
			...
			Local $a = decodeSingleCoord(FindImageInPlace("GoogleSelectEmail", $g_sImgGoogleSelectEmail, "220,44(400,600)", False)) ; RC Done
			If UBound($a) > 1 Then
		...
		For $i = 0 To 10
			...
			If UBound($XCoordinates) > 0 Then
				...
				For $j = 0 To UBound($XCoordinates) - 1
					Local $Coordinates = StringSplit($XCoordinates[$j], ",", 2)
					; Let's check duplicated coordinates
					SetDebugLog("Coordinate Y: " & Number($Coordinates[1]) + 165)
					SetDebugLog("Last Y: " & $YLastCoordinate)
					If (Number($Coordinates[1]) + 165 > $YLastCoordinate + 25 Or Number($Coordinates[1]) + 165 < $YLastCoordinate - 25) Then
						Local $Duplicated = False
						For $x = 0 To UBound($AccountsCoord) - 1
							If $AccountsCoord[$x][1] < Number($Coordinates[1]) + 165 + 25 And $AccountsCoord[$x][1] > Number($Coordinates[1]) + 165 - 25 Then $Duplicated = True
						Next
						If Not $Duplicated Then
							ReDim $AccountsCoord[UBound($AccountsCoord) + 1][2]
							$AccountsCoord[UBound($AccountsCoord) - 1][0] = $Coordinates[0] + 550
							$AccountsCoord[UBound($AccountsCoord) - 1][1] = $Coordinates[1] + 165
							$YLastCoordinate = $Coordinates[1] + 165
						EndIf
					EndIf
				Next
				_ArraySort($AccountsCoord, 0, 0, 0, 1) ; short by column 1 [Y]
				...
				If $acount + 1 > UBound($AccountsCoord) Then
					setlog("You selected a SCID undetected account!!", $COLOR_ERROR)
					ExitLoop
				EndIf
				Click($AccountsCoord[$acount][0] - 250, $AccountsCoord[$acount][1], 1)
				SetLog("Please wait for loading CoC...!")
				ExitLoop
			EndIf
