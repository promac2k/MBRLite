...
Func ProfileReport()
	...
	While Not _ColorCheck(_GetPixelColor(258, 79, True), Hex(0xcdcbbc, 6), 5) ; wait for Info Profile to open
		$iCount += 1
		If _Sleep($DELAYPROFILEREPORT1) Then Return
	...
	For $i = 0 To 1 ; Check twice,  because the button is animated
		If QuickMIS("BC1", $g_sImgCollectReward, 680, 165 + $g_iMidOffsetYNew, 855, 680 + $g_iBottomOffsetYNew) Then ; RC Done
	...
	If _ColorCheck(_GetPixelColor($aProfileReport[0], $aProfileReport[1], True), Hex($aProfileReport[2], 6), $aProfileReport[3]) Then
		...
		$iDefensesWon = 0
	Else
		$iAttacksWon = getProfile(578, 347 + $g_iMidOffsetYNew) ; RC Done
		...
		WEnd
		If $g_bDebugSetlog And $iCount >= 20 Then SetLog("Excess wait time for reading $AttacksWon: " & getProfile(578, 347 + $g_iMidOffsetYNew), $COLOR_DEBUG)
		$iDefensesWon = getProfile(790, 347 + $g_iMidOffsetYNew) ; RC Done
	EndIf
	$g_iTroopsDonated = getProfile(158, 347 + $g_iMidOffsetYNew) ; RC Done
	$g_iTroopsReceived = getProfile(360, 347 + $g_iMidOffsetYNew) ; RC Done
	...
	Click(830, 40, 1, 0, "#0223") ; Close Profile page
