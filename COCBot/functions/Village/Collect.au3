...
Func CollectLootCart()
	...
	If IsArray($Var) And $Var[0] Then
		Local $aNewLootCart = _ImageSearchXMLMultibot($g_sImgCollectNewLootCart, $Quantity2Match, $Area2Search, $ForceArea, False)
		...
		Else
			SetDebugLog("New LootCart Image Failed! Procedding with old image.", $COLOR_DEBUG)
	...
	If $bCollectWithOldMethod Then
		...
		SetDebugLog("$aLootCart Old Method: " & Round(TimerDiff($Time)) & "'ms")
