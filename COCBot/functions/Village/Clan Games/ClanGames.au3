...
; Modified ......: ProMac 02/2018 [v2 and v3] , ProMac 12/2018 v4
...
Func _ClanGames($test = False)
	...
	__ClanGames($test)
	; Need to Check Clan Games Window Before Returning From Clan Games Function
	CloseClanGamesWindow()
EndFunc   ;==>_ClanGames
; Main Loop Function Don't Call it directily Call _ClanGames
Func __ClanGames($test = False)
	...
	Local $TabChallengesPosition[2] = [300, 30] ; RC Done
	...
	Local $Rows = ["300,111,760,201", "300,271,760,361", "300,431,760,506"] ; RC Done
				...
				Case "B"
					...
					For $j = 0 To UBound($BattleChallenges) - 1
						...
						If $aAllDetectionsOnScreen[$i][1] = $BattleChallenges[$j][0] Then
							...
							If $BattleChallenges[$j][1] = "Scrappy 6s" And $g_iTownHallLevel > 7 Or $g_iTownHallLevel < 4 Then ExitLoop ; TH level 5-6-7
							If $BattleChallenges[$j][1] = "Super 7s" And $g_iTownHallLevel > 8 Or $g_iTownHallLevel < 5 Then ExitLoop ; TH level 6-7-8
							If $BattleChallenges[$j][1] = "Exciting 8s" And $g_iTownHallLevel > 9 Or $g_iTownHallLevel < 6 Then ExitLoop ; TH level 7-8-9
							If $BattleChallenges[$j][1] = "Noble 9s" And $g_iTownHallLevel > 10 Or $g_iTownHallLevel < 7 Then ExitLoop ; TH level 8-9-10
							If $BattleChallenges[$j][1] = "Terrific 10s" And $g_iTownHallLevel > 11 Or $g_iTownHallLevel < 8 Then ExitLoop ; TH level 9-10-11
							If $BattleChallenges[$j][1] = "Exotic 11s" And $g_iTownHallLevel > 12 Or $g_iTownHallLevel < 9 Then ExitLoop ; TH level 10-11-12
							If $BattleChallenges[$j][1] = "Triumphant 12s" And $g_iTownHallLevel > 13 Or $g_iTownHallLevel < 10 Then ExitLoop ; TH level 11-12-13
							...
							If $BattleChallenges[$j][1] = "Attack Up" And $g_iTownHallLevel > 11 Then ExitLoop
	...
	If UBound($aSelectChallenges) > 0 Then
		...
		If _sleep(250) Then Return
		ClickDrag(807, 165, 807, 340, 500) ; RC Done
		If _Sleep(500) Then Return
	EndIf
...
;Clan Games Window Is Edge to Edge Now After 4:3 So Added Additional Check To Close
Func CloseClanGamesWindow()
	;Just In Case Wait for 200ms If It Find Clan Game Window Close Pixel Just Close It Otherwise
	;In Else Click away will close the other clan related windows
	If _Wait4Pixel(827, 34, 0xFFFFFF, 5, 200, "CloseClanGamesWindow") Then
		Click(827, 34) ;Close Window
	Else
...
Func IsClanGamesWindow($getCapture = True)
	If QuickMIS("BC1", $g_sImgCaravan, 200, 55, 300, 135, $getCapture, False) Then ; RC Done
		...
		If _Sleep(1500) Then Return
		If QuickMIS("BC1", $g_sImgReward, 760, 480 + $g_iMidOffsetYNew, 830, 570 + $g_iMidOffsetYNew, $getCapture, $g_bChkClanGamesDebug) Then ; RC Done
			SetLog("Your Reward is Ready", $COLOR_INFO)
			CloseClanGamesWindow()
			If _Sleep(100) Then Return
			Return False
		EndIf
		If _ColorCheck(_GetPixelColor(384, 388 + $g_iMidOffsetYNew, True), Hex(0xFFFFFF, 6), 5) Then ; RC Done
			Local $sTimeRemain = getOcrTimeGameTime(380, 461 + $g_iMidOffsetYNew) ; read Clan Games waiting time ; RC Done
...
Func GetTimesAndScores()
	...
	$sTimeRemain = StringReplace(getOcrTimeGameTime(50, 479 + $g_iMidOffsetYNew), " ", "") ; RC Done ; read Clan Games waiting time
	; JUST IN CASE

	If Not _IsValideOCR($sTimeRemain) Then
		If _ColorCheck(_GetPixelColor(232, 344 + $g_iMidOffsetYNew, True), Hex(0x4c84c4, 6), 5) Then ; RC Done
			SetLog("You Need to Joing a Clan!!!", $COLOR_WARNING)
		Else
	...
	For $i = 0 To 10
		$YourScore = getOcrYourScore(55, 533 + $g_iMidOffsetYNew) ;  Read your Score
...
Func CooldownTime($getCapture = True)
	; Check IF exist the Gem icon
	;check cooldown purge
	If QuickMIS("BC1", $g_sImgCoolPurge, 480, 370 + $g_iMidOffsetYNew, 570, 410 + $g_iMidOffsetYNew, $getCapture, False) Then ; RC Done
...
Func IsEventRunning()
	...
	If Not _ColorCheck(_GetPixelColor(304, 257 + $g_iMidOffsetYNew, True), Hex(0x53E050, 6), 5) Then ; RC Done ; Green Bar from First Position
		SetLog("An Event is already in progress !", $COLOR_SUCCESS)
		If $g_bChkClanGamesDebug Then SetLog("[0]: " & _GetPixelColor(304, 257 + $g_iMidOffsetYNew, True)) ; RC Done
		CloseClanGamesWindow()
		Return True
	Else
...
Func StartsEvent($sEventName, $g_bPurgeJob = False, $getCapture = True, $g_bChkClanGamesDebug = False)
	...
	; Start an Event
	If $g_bRunState = False Then Return
	If QuickMIS("BC1", $g_sImgStart, 220, 150 + $g_iMidOffsetYNew, 830, 580 + $g_iMidOffsetYNew, $getCapture, $g_bChkClanGamesDebug) Then ; RC Done
		Local $Timer = GetEventTimeInMinutes($g_iQuickMISWOffSetX, $g_iQuickMISWOffSetY)
		...
		If $g_bPurgeJob Then
			If _Sleep(2000) Then Return
			; Click($g_iQuickMISWOffSetX, $g_iQuickMISWOffSetY)
			If QuickMIS("BC1", $g_sImgTrashPurge, 220, 150 + $g_iMidOffsetYNew, 830, 580 + $g_iMidOffsetYNew, $getCapture, $g_bChkClanGamesDebug) Then ; RC Done
				Click($g_iQuickMISWOffSetX, $g_iQuickMISWOffSetY)
				If _Sleep(1200) Then Return
				SetLog("Click Trash", $COLOR_INFO)
				If QuickMIS("BC1", $g_sImgOkayPurge, 440, 400 + $g_iMidOffsetYNew, 580, 450 + $g_iMidOffsetYNew, $getCapture, $g_bChkClanGamesDebug) Then ; RC Done
...
Func PurgeEvent($directoryImage, $sEventName, $getCapture = True)
	...
	If QuickMIS("BC1", $directoryImage, $x, $y + $g_iMidOffsetYNew, $x1, $y1 + $g_iMidOffsetYNew, $getCapture, $g_bChkClanGamesDebug) Then ; RC Done
		Click($g_iQuickMISWOffSetX, $g_iQuickMISWOffSetY)
...
Func GetEventInformation()
	If QuickMIS("BC1", $g_sImgStart, 220, 150 + $g_iMidOffsetYNew, 830, 580 + $g_iMidOffsetYNew, True, $g_bChkClanGamesDebug) Then ; RC Done
		Return GetEventTimeInMinutes($g_iQuickMISWOffSetX, $g_iQuickMISWOffSetY) ; RC Done
	EndIf
	Return 0
