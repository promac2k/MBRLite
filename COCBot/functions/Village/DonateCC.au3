...
Func DonateCC($bCheckForNewMsg = False)
	...
	;Check if any windows showing on top of main screen
	CloseDonateWindowIfShown()
	...
	WEnd
	; December Update(2018) Check for "I Understand" Warning
	Local $aButtonCoords = decodeSingleCoord(findImage("I Understand", $g_sImgChatIUnterstand, GetDiamondFromRect("30,336,285,516"))) ; RC Done
	...
	While $bDonate
		...
		$g_aiDonatePixel = _MultiPixelSearch(200, $y, 230, 590, -2, 1, Hex(0x6da725, 6), $aChatDonateBtnColors, 20)
		...
		;;; Scroll Down
		ForceCaptureRegion()
		$Scroll = _PixelSearch(293, 563, 295, 571, Hex(0xFFFFFF, 6), 20)
	...
	While 1
		...
		If _ColorCheck(_GetPixelColor($aCloseChat[0], $aCloseChat[1], True), Hex($aCloseChat[2], 6), $aCloseChat[3]) Then
			...
			ExitLoop
			;sometimes the Donate troops window doesn't close and is necessary check the red [x]
...
Func CloseDonateWindowIfShown()
	;on 860x644 SomeTime Close Window is on top direct Click Away Does not close that windows so check if donate window showing close that else click away
	If QuickMIS("BC1", $g_sImgDonateCloseWindow, 790, 0, 825, 220, True, False) Then ; RC Done
		SetLog("Closing the Donate troops window!...", $COLOR_SUCCESS)
		Click($g_iQuickMISWOffSetX, $g_iQuickMISWOffSetY)
		;Add delay so donate window will get close
...
Func DetectSlotTroop(Const $iTroopIndex)
	...
	For $Slot = 7 To 13
		...
		If $FullTemp[0] <> "" Then
			For $i = $eTroopBalloon To $eTroopIceGolem
				Local $iFoundTroopIndex = TroopIndexLookup($FullTemp[0])
				If $iFoundTroopIndex = $i Then
					If $g_bDebugSetlog Then SetDebugLog("Detected " & $g_asTroopNames[$i], $COLOR_DEBUG)
					If $iTroopIndex = $i Then Return $Slot
