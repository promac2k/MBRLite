...
Func BotCommand()
	...
	SetDebugLog("Halt is selected : " & $g_bChkBotStop, $COLOR_INFO)
	SetDebugLog(" - What Condition: " & GUICtrlRead($g_hCmbBotCond) & " : " & $iCmbBotCond, $COLOR_INFO)
	SetDebugLog(" - What to do: " & GUICtrlRead($g_hCmbBotCommand) & " : " & $iCmbBotCommand, $COLOR_INFO)
	...
	If $bChkBotStop = True Then
		...
		Switch $iCmbBotCond
			...
			Case 21 ; Have shield - Online/Collect
				...
				EndIf
			Case Else
				SetLog("Something Wrong CmbBotCond...", $COLOR_WARNING)
				Return False
		EndSwitch
		...
		If $g_bMeetCondStop Then
			Switch $iCmbBotCommand
				...
				Case 1
					SetLog("MultiBot.run Bot Stop as requested!!", $COLOR_INFO)
					If _Sleep($DELAYBOTCOMMAND1) Then Return
					Return True
				Case 2
					SetLog("MultiBot.run Close Bot as requested!!", $COLOR_INFO)
					If _Sleep($DELAYBOTCOMMAND1) Then Return
					BotClose()
				...
				Case 6
					...
					Return True ; HaHa - No Return possible!
				Case Else
					SetLog("Something Wrong CmbBotCommand...", $COLOR_WARNING)
					Return False
			EndSwitch
		EndIf
	EndIf
