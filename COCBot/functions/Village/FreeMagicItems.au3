...
Func CollectFreeMagicItems($bTest = False)
	...
	If QuickMIS("BC1", $g_sImgTrader, 120, 140, 210, 215, True, False) Then ; RC Done
	...
	If Not QuickMIS("BC1", $g_sImgDailyDiscountWindow, 280, 175 + $g_iMidOffsetYNew, 345, 210 + $g_iMidOffsetYNew, True, False) Then ; RC Done
		ClickP($aAway, 1, 0, "#0332") ;Click Away
		Return
	EndIf

	If Not $g_bRunState Then Return
	Local $aOcrPositions[3][2] = [[200, 439 + $g_iMidOffsetYNew], [390, 439 + $g_iMidOffsetYNew], [580, 439 + $g_iMidOffsetYNew]] ; RC Done
	Local $aResults[3] = ["", "", ""]

	$iLastTimeChecked[$g_iCurAccount] = @MDAY

	For $i = 0 To 2
		$aResults[$i] = getOcrAndCapture("coc-freemagicitems", $aOcrPositions[$i][0], $aOcrPositions[$i][1], 80, 25, True) ; RC Done
