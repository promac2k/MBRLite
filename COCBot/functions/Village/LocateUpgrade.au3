...
Func UpgradeValue($inum, $bRepeat = False) ;function to find the value and type of the upgrade.
	...
	If IsArray($aResult) Then
		...
		EndIf
	Else
		SetLog("ErrorOcr: Name & Level for Upgrade not found?", $COLOR_ERROR)
	EndIf
	...
	If QuickMIS("BC1", $g_sImgAUpgradeUpgradeBtn, 120, 608 + $g_iBottomOffsetYNew, 740, 670 + $g_iBottomOffsetYNew) Then ; RC Done
		...
		Select ;Ensure the right upgrade window is open!
			Case _ColorCheck(_GetPixelColor(687, 161 + $g_iMidOffsetY), Hex(0xCD1419, 6), 20) ; Check if the building Upgrade window is open red bottom of white X to close
				...
				$g_avBuildingUpgrades[$inum][6] = getBldgUpgradeTime(190, 307 + $g_iMidOffsetY) ; Try to read white text showing time for upgrade
			...
			Case _ColorCheck(_GetPixelColor(719, 118 + $g_iMidOffsetY), Hex(0xDF0408, 6), 20) ; Check if the Hero Upgrade window is open
				...
				$g_avBuildingUpgrades[$inum][2] = Number(getResourcesBonus(598, 509)) ; Try to read white text.
				If $g_avBuildingUpgrades[$inum][2] = "" Then $g_avBuildingUpgrades[$inum][2] = Number(getUpgradeResource(598, 509)) ;read RED upgrade text
				...
				$g_avBuildingUpgrades[$inum][6] = getHeroUpgradeTime(578, 450) ; Try to read white text showing time for upgrade
				SetLog("Upgrade #" & $inum + 1 & " Time = " & $g_avBuildingUpgrades[$inum][6], $COLOR_INFO)
				If $g_avBuildingUpgrades[$inum][6] <> "" Then $g_avBuildingUpgrades[$inum][7] = "" ; Clear old upgrade end time
