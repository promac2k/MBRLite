...
Func BoostPotion($sName, $sOcrName, $aPos, ByRef $icmbBoostValue, $cmbBoostCtrl)
	...
	EndIf
	If $ok = True Then
		Local $sTile = "BoostPotion_0_90.xml", $sRegionToSearch = "172,194,684,425" ; RC Done
		...
		If UBound($Boost) > 1 Then
			...
			If UBound($Boost) > 1 Then
				...
				ClickP($Boost)
				If _Sleep($DELAYBOOSTHEROES1) Then Return
				If Not _ColorCheck(_GetPixelColor(255, 550+ $g_iMidOffsetYNew, True), Hex(0xFFFFFF, 6), 25) Then ; RC Done
