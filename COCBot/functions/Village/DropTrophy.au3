				...
				If $g_bDropTrophyAtkDead Then
					...
					If $G = True And $E = True And $GPE = True Then
						...
						If checkDeadBase() Then
							...
							PrepareAttack($g_iMatchMode) ; ==== Troops :checks for type, slot, and quantity ===
							...
							Switch $t
								Case "Q"
									If $g_iQueenSlot <> -1 Then
										...
										Click(GetXPosOfArmySlot($g_iQueenSlot, True), 595 + $g_iBottomOffsetY, 1, 0, "#0179") ; RC Done ;Select Queen
								...
								Case "K"
									If $g_iKingSlot <> -1 Then
										...
										Click(GetXPosOfArmySlot($g_iKingSlot, True), 595 + $g_iBottomOffsetY, 1, 0, "#0177") ; RC Done ;Select King
										If _Sleep($DELAYDROPTROPHY1) Then ExitLoop
								...
								Case "W"
									If $g_iWardenSlot <> -1 Then
										...
										Click(GetXPosOfArmySlot($g_iWardenSlot, True), 595 + $g_iBottomOffsetY, 1, 0, "#0000") ; RC Done ;Select Warden
										If _Sleep($DELAYDROPTROPHY1) Then ExitLoop
				...
				If ($g_iQueenSlot = -1 And $g_iKingSlot = -1 And $g_iWardenSlot = -1) Or Not $g_bDropTrophyUseHeroes Then
					...
					For $i = 0 To UBound($g_avAttackTroops) - 1
						If ($g_avAttackTroops[$i][0] >= $eBarb And $g_avAttackTroops[$i][0] <= $eWiza) Or $g_avAttackTroops[$i][0] = $eMini Then
							...
							$bIsDropTroopFound = True
							ExitLoop
						EndIf
					Next
					If Not $bIsDropTroopFound Then
