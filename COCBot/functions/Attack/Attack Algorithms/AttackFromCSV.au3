...
Local $InnerDiamandDiffX = 50 ; set the diamond shape based on reference village
Local $InnerDiamandDiffY = 38 ; set the diamond shape based on reference village
...
Func Algorithm_AttackCSV($testattack = False, $captureredarea = True)
	...
	; Just to upadte the External and Internal Areas
	ConvertInternalExternArea("Algorithm_AttackCSV")
	...
	If $g_bCSVLocateStorageDarkElixir = True Then
		$hTimer = __timerinit()
		SuspendAndroid()
		Local $g_aiPixelDarkElixirStorage = GetLocationDarkElixirStorage()
		ResumeAndroid()
		If _Sleep($DELAYRESPOND) Then Return
	...
	; 14 - START TH SNIPE BEFORE ATTACK CSV IF NEED ------------------------------------------
