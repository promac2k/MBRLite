...
Func ReturnHome($TakeSS = 1, $GoldChangeCheck = True) ;Return main screen
	...
	For $i = 0 To 30
		...
		If _CheckPixel($aEndFightSceneAvl, $g_bCapturePixel) Then ; check for the gold ribbon in the end of battle data screen : RC
			...
			EndIf
		EndIf
		If $i = 30 Then SetDebugLog("Wait for End Fight Scene to appear #" & $i & " error!") ; if end battle window is not found in 30*200mms or 6 seconds, then give up.
		If _Sleep($DELAYRETURNHOME5) Then Return
	Next

	If _Sleep($DELAYRETURNHOME4) Then Return ; short wait for screen to close
	If checkObstacles() Then checkMainScreen(False) ; trap common error messages without setlog
	...
	For $i = 0 To 50
		...
		If $i = 50 Or isProblemAffect(True) Then
