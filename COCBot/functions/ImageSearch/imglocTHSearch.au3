...
Func imglocTHSearch($bReTest = False, $myVillage = False, $bForceCapture = True)
	...
	;This veriable is to use Offset for Centerting Townhall according to there template
	;NOTE: When ever template of townhall changed need to update offset as it can cause issue if template postion was changed
	;X,Y,objectname_objectlevel
	Local $aTHCenterOffset = [ _
			[-8, -20, "TownHall_12"], _ ;TownHall_12_90.xml
			[-3, 8, "th_12"], _ ;[5, -2, "th_12"], _ ;th_12_90.xml
			[9, 1, "th_11"] _ ;th_11_90.xml
			]
	...
	;Don't change this order of return props as getting used for $aTHCenterOffset
					...
					Switch $propsNames[$pv]
						...
						Case "objectpoints"
							...
							If $myVillage = False Then
								...
								If UBound($IMGLOCTHLOCATION) > 1 Then ;Just In Case Coord Not Coorect
									;Check if Offset Found for centering Th
									Local $iTHCenterOffsetIndex = _ArraySearch($aTHCenterOffset, $propsValues[0] & "_" & $propsValues[1]) ;objectname_objectlevel
									If $iTHCenterOffsetIndex <> -1 Then
										If $g_bDebugSetlog Then SetDebugLog("imgloc Found : Center Offset Old Pos(" & Number($IMGLOCTHLOCATION[0]) & ", " & Number($IMGLOCTHLOCATION[1]) & ") -> New Pos(" & Number($IMGLOCTHLOCATION[0]) + $aTHCenterOffset[$iTHCenterOffsetIndex][0] & ", " & Number($IMGLOCTHLOCATION[1]) + $aTHCenterOffset[$iTHCenterOffsetIndex][1] & ")", $COLOR_INFO)
										$IMGLOCTHLOCATION[0] = Number($IMGLOCTHLOCATION[0]) + $aTHCenterOffset[$iTHCenterOffsetIndex][0]
										$IMGLOCTHLOCATION[1] = Number($IMGLOCTHLOCATION[1]) + $aTHCenterOffset[$iTHCenterOffsetIndex][1]
										;Need to update $propsValues[$pv] as getting used for decodeMultipleCoords
										$propsValues[$pv] = $IMGLOCTHLOCATION[0] & "," & $IMGLOCTHLOCATION[1]
									EndIf
							...
							Else
								...
								If UBound($g_aiTownHallPos) > 1 Then ;Just In Case Coord Not Coorect
