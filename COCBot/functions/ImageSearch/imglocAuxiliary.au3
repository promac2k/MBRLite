...
Func GetButtonDiamond($sButtonName)
	...
	Switch $sButtonName
		Case "FindMatch" ;Find Match Screen
			$btnDiamond = "117,453|316,453|316,545|117,545" ; RC Done
		Case "CloseFindMatch" ;Find Match Screen
			$btnDiamond = "806,12|848,12|848,52|806,52" ; RC Done
		Case "Attack" ;Main Window Screen
			$btnDiamond = "15,532|112,532|112,627|15,627" ; RC Done
		Case "OpenTrainWindow" ;Main Window Screen
			$btnDiamond = "15,472|65,472|65,522|15,522" ; RC Done
		Case "OK"
			$btnDiamond = "440,351|587,351|587,416|440,416" ; RC Done
		Case "CANCEL"
			$btnDiamond = "272,351|420,351|420,416|272,416" ; RC Done
		Case "ReturnHome"
			$btnDiamond = "357,501|502,501|502,563|357,563" ; RC Done
		Case "Next" ; attackpage attackwindow
			$btnDiamond = "697,454|850,454|850,522|697,522" ; RC Done
		Case "ObjectButtons", "BoostOne", "BoostCT" ; Full size of object buttons at the bottom
			$btnDiamond = GetDiamondFromRect("200,505(460,75)") ; RC Done
		Case "GEM", "BOOSTBtn" ; Boost window button (full button size)
			$btnDiamond = GetDiamondFromRect("359,368(148,66)") ; RC Done
		Case "EnterShop"
			$btnDiamond = GetDiamondFromRect("359,348(148,66)") ; RC Done
		Case "EndBattleSurrender" ;surrender - attackwindow
			$btnDiamond = "12,489|125,489|125,527|12,527" ; RC Done
		Case "ExpandChat" ;mainwindow
			$btnDiamond = "2,286|35,306|35,366|2,386" ; RC Done
		Case "CollapseChat" ;mainwindow
			$btnDiamond = "315,290|350,306|350,366|315,386" ; RC Done
		Case "ChatOpenRequestPage" ;mainwindow - chat open
			$btnDiamond = "5,600|65,600|65,637|5,637" ; RC Done
		...
		Case "DonateWindow" ;mainwindow - only when donate window is visible
			$btnDiamond = "310,0|360,0|360,644|310,644" ; RC Done
		Case "DonateButton" ;mainwindow - only when chat window is visible
			$btnDiamond = "200,85|305,85|305,590|200,590" ; RC Done
		...
		Case "DownDonation" ;mainwindow - only when chat window is visible
			$btnDiamond = "282,547|306,547|306,590|282,590" ; RC Done
		Case "Treasury"
			$btnDiamond = "125,505|740,505|740,580|125,580" ; RC Done
		Case "Collect"
			$btnDiamond = "350,410|505,410|505,480|350,480" ; RC Done
		Case "BoostBarrack", "BarrackBoosted"
			$btnDiamond = GetDiamondFromRect("630,236,850,316") ; RC Done
		Case "ArmyTab", "TrainTroopsTab", "BrewSpellsTab", "BuildSiegeMachinesTab", "QuickTrainTab"
			$btnDiamond = GetDiamondFromRect("18,56,800,106") ; RC Done
		Case "Research"
			$btnDiamond = "110,505|730,505|750,580|111,580" ; RC Done
		Case Else
...
Func findMultiple($directory, $sCocDiamond, $redLines, $minLevel = 0, $maxLevel = 1000, $maxReturnPoints = 0, $returnProps = "objectname,objectlevel,objectpoints", $bForceCapture = True)
	...
	Local $aCoords = "" ; use AutoIt mixed variable type and initialize array of coordinates to null
	Local $aWhatObjProperty = StringSplit($returnProps, ",", $STR_NOCOUNT)
	Local $aRetriveObjProperty[UBound($aWhatObjProperty)]
	; [X] is an array inside
	Local $aReturnValues[0]
	...
	If $result[0] <> "" Then ;despite being a string, AutoIt receives a array[0]
		Local $aAllTilesFound = StringSplit($result[0], "|", $STR_NOCOUNT)
		For $iEachTile = 0 To UBound($aAllTilesFound) - 1
			For $iObjProperty = 0 To UBound($aWhatObjProperty) - 1 ; cycle props
				$aRetriveObjProperty[$iObjProperty] = RetrieveImglocProperty($aAllTilesFound[$iEachTile], $aWhatObjProperty[$iObjProperty])
				If $aWhatObjProperty[$iObjProperty] = "objectpoints" Then
					; validate object points
					If StringInStr($aRetriveObjProperty[$iObjProperty], ",") = 0 Then
						; no valid coordinate, skip values and goes to next Tile found
						If $g_bDebugSetlog Then SetDebugLog("findMultiple : Invalid objectpoint in result in " & $iObjProperty & ": " & $result[0])
						ContinueLoop 2
					EndIf
				EndIf
				If $g_bDebugSetlog Then SetDebugLog("findMultiple : " & $aAllTilesFound[$iEachTile] & "->" & $aWhatObjProperty[$iObjProperty] & " -> " & $aRetriveObjProperty[$iObjProperty])
			Next
			; Is a valid Tile with coordinates let's redim and make a new Dimension and assign the array - $aRetriveObjProperty[X] to return
			ReDim $aReturnValues[UBound($aReturnValues) + 1]
			$aReturnValues[UBound($aReturnValues) - 1] = $aRetriveObjProperty
		Next
...
Func Slot($iX, $iY) ; Return Slots for Quantity Reading on Army Window
	If $iY < 445 Then ; RC Done
		Switch $iX ; Troops & Spells Slots
			Case 0 To 94 ; Slot 1
				If $iY < 270 Then Return 35 ; RC Done ; Troops
				If $iY > 270 Then Return 40 ; RC Done ; Spells

			Case 95 To 170 ; Slot 2
				If $iY < 270 Then Return 111 ; RC Done ; Troops
				If $iY > 270 Then Return 120 ; RC Done ; Spell

			Case 171 To 243 ; Slot 3
				If $iY < 270 Then Return 184 ; RC Done ; Troops
				If $iY > 270 Then Return 195 ; RC Done ; Spell

			Case 244 To 307 ; Slot 4
				If $iY < 270 Then Return 255 ; RC Done ; Troops
				If $iY > 270 Then Return 272 ; RC Done ; Spell

			Case 308 To 392 ; Slot 5
				If $iY < 270 Then Return 330 ; RC Done ; Troops
				If $iY > 270 Then Return 341 ; RC Done ; Spell

			Case 393 To 464 ; Slot 6
				If $iY < 270 Then Return 403 ; RC Done ; Troops
				If $iY > 270 Then Return 415 ; RC Done ; Spell

			Case 465 To 540 ; Slot 7
				If $iY < 270 Then Return 477 ; RC Done ; Troops
				If $iY > 270 Then Return 485 ; RC Done ; Spell
			...
			Case 683 To 752 ; Slot 10
				If $iY > 270 Then Return 691 ; RC Done ; Heroes & Siege Machine slot 0
				Return 700 ; Siege Machines slot 1
			Case 753 To 822 ; Slot 11
				Return 773 ; Siege Machines slot 2
		EndSwitch
