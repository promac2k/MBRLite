...
Func checkObstacles($bBuilderBase = False) ;Checks if something is in the way for mainscreen
	...
	EndIf
	If _ColorCheck(_GetPixelColor(383, 405 + $g_iMidOffsetYNew), Hex(0xF0BE70, 6), 20) Then ; RC Done
...
Func _checkObstacles($bBuilderBase = False, $bRecursive = False) ;Checks if something is in the way for mainscreen
	...
	If IsArray($aMessage) Or (UBound(decodeSingleCoord(FindImageInPlace("Error", $g_sImgError, "680,256(2,20)", False, $g_iAndroidLollipop))) > 1) Then ; RC Done
		If $g_bDebugSetlog Then SetDebugLog("(DC=" & _GetPixelColor($aIsConnectLost[0], $aIsConnectLost[1] + $g_iMidOffsetYNew) & ")(OoS=" & _GetPixelColor($aIsCheckOOS[0], $aIsCheckOOS[1] + $g_iMidOffsetYNew) & ")", $COLOR_DEBUG) ; RC Done
		If $g_bDebugSetlog Then SetDebugLog("(Maintenance=" & _GetPixelColor($aIsMaintenance[0], $aIsMaintenance[1] + $g_iMidOffsetYNew) & ")(RateCoC=" & ")", $COLOR_DEBUG) ; RC Done
		...
		Local $sRegion = "220,286(80,60)" ; RC Done
		...
		If UBound(decodeSingleCoord(FindImageInPlace("Break", $g_sImgPersonalBreak, "165,243,335,291", False))) > 1 Then ; RC Done ; used for all 3 different break messages
		...
		;;;;;;;##### Important Notice - SC ToS #####;;;;;;;
		If UBound(decodeSingleCoord(FindImageInPlace("Notice", $g_sImgNotice, "150,216,250,60", False))) > 1 Then
			SetLog("Important Notice, Super Cell ToS...", $COLOR_ERROR)
			PureClick(280, 450, 1, 0, "#9999") ; Click on never to close window and stop reappear. Never=248,408 & Later=429,408
			$g_bMinorObstacle = True
			If _Sleep($DELAYCHECKOBSTACLES1) Then Return
			Return True
		EndIf
		...
		Select
			Case UBound(decodeSingleCoord(FindImageInPlace("AnyoneThere", $g_sImgAnyoneThere, "440,296,580,346", False))) > 1 ; RC Done ; Inactive only
			...
			Case _CheckPixel($aIsConnectLost, $g_bNoCapturePixel) ; Connection Lost
				...
				If $g_bChkSharedPrefs And HaveSharedPrefs() Then
					...
					Return True
				EndIf
			Case _CheckPixel($aIsCheckOOS, $g_bNoCapturePixel) Or (UBound(decodeSingleCoord(FindImageInPlace("OOS", $g_sImgOutOfSync, "355,291,435,351", False, $g_iAndroidLollipop))) > 1) ; RC Done ; Check OoS
			...
			Case Else
				...
				Local $sRegion = "220,360(60,25)"             ; RC Done
				If $g_iAndroidVersionAPI >= $g_iAndroidLollipop Then
					$sRegion = "555,340(60,25)" ; RC Done
				EndIf
	...
	; let's check first if you are on Main Village or Builder Base Main Window
	If _CheckPixel($aIsMain, $g_bNoCapturePixel) Or _CheckPixel($aIsOnBuilderBase, $g_bNoCapturePixel) Then Return ; If its main screen return
	...
	If _ColorCheck(_GetPixelColor(792, 39), Hex(0xDC0408, 6), 20) Then
		...
		Return False
	EndIf
	;Cords Choosed Very Carefully Because all 3 "X" Are at slight different position but these pixels exist in all 3 close windows
	If _ColorCheck(_GetPixelColor(825, 30), Hex(0xFFFFFF, 6), 5) And _ColorCheck(_GetPixelColor(836, 22), Hex(0xFD8585, 6), 25) Then   ;Clan Games,Attack Screen,Upgrade Window Close [X] White Pixel And Red Pixel
		SetDebugLog("checkObstacles: Found Window with Close Button to close")
		PureClick(825, 30, 1, 0, "#0134") ;Clicks X
		$g_bMinorObstacle = True
		If _Sleep($DELAYCHECKOBSTACLES1) Then Return
		Return False
	EndIf
	...
	;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	If _CheckPixel($aEndFightSceneBtn, $g_bNoCapturePixel) And _CheckPixel($aEndFightSceneAvl, $g_bCapturePixel) And Not $bBuilderBase Then
		...
		Return True
	EndIf
	If _CheckPixel($aSurrenderButton, $g_bNoCapturePixel) And Not $bBuilderBase Then
	...
	Local $CSFoundCoords = decodeSingleCoord(FindImageInPlace("CocStopped", $g_sImgCocStopped, "250,314,618,388", False)) ; RC Done
...
Func checkObstacles_ResetSearch()
	If $g_bDebugSetlog Then SetDebugLog("Inside checkObstacles_ResetSearch", $COLOR_DEBUG)
...
Func checkObstacles_Network($bForceCapture = False, $bReloadCoC = True)
	...
	If UBound(decodeSingleCoord(FindImageInPlace("CocReconnecting", $g_sImgCocReconnecting, "420,311,440,331", $bForceCapture))) > 1 Then ; RC Done
