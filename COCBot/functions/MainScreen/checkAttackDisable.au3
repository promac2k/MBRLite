...
; Parameters ....: $Result              - [optional] previous saved string from OCR read. Default is getAttackDisable(346, 156) or getAttackDisable(180,156)
...
Func checkAttackDisable($iSource, $Result = "")
	...
	Switch $iModSource
		Case $g_iTaBChkAttack ; look at location 346, 156 for "disable", "for" or "after" if checked early enough
			While $Result = "" Or (StringLen($Result) < 3)
				$i += 1
				If _Sleep($DELAYATTACKDISABLE100) Then Return
				$Result = getAttackDisable(346, 156) ; Grab Ocr for TakeABreak if not found due slow PC
			...
			EndIf
		Case $g_iTaBChkIdle ; look at location 180, 156 for the have "been" online too long message, and the "after" Personal Break message
			If $Result = "" Then $Result = getAttackDisable(180, 156) ; change to 156, 860*644
			...
			If $Result <> "" Then ; fast test to see if have Take-A-Break
				If StringInStr($Result, "been") <> 0 Or StringInStr($Result, "after") <> 0 Or StringInStr($Result, "have") <> 0 Then ; verify we have right text string, 'have' added for Personal Break
		...
		Case $g_iTaBChkTime
			...
			If $iSource = $g_iTaBChkIdle Then
				...
				If _Sleep($DELAYATTACKDISABLE500) Then Return
			EndIf
			If $g_asShieldStatus[0] = "guard" And Int($g_aiCurrentLoot[$eLootTrophy]) < 5000 Then
	...
	; CoC is closed >>
	; For Legends Guard System Is Changed That's Why Check Added That If Guy is on legend even if he has guard it's personal break will be about to come
	If $iModSource = $g_iTaBChkTime And ($g_asShieldStatus[0] <> "guard" Or Int($g_aiCurrentLoot[$eLootTrophy]) > 5000) Then
