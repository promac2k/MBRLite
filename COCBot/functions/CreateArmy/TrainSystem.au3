...
Func TrainSystem()
	...
	If Not $g_bDoubleTrainDone Or Not $g_bDoubleTrain Then TrainSiege()
	DoubleTrain($g_bQuickTrainEnable)
...
Func TestMaxCamp()
	...
	If _Sleep(250) Then Return
	Local $ArmyCamp = GetOCRCurrent(48, 160 + $g_iMidOffsetYNew) ; RC Done
...
Func BrewUsingWhatToTrain($Spell, $Quantity) ; it's job is a bit different with 'TrainUsingWhatToTrain' Function, It's being called by TrainusingWhatToTrain Func
	...
	Select
		Case $g_bIsFullArmywithHeroesAndSpells = False
			If Not _ColorCheck(_GetPixelColor(230, 208 + $g_iMidOffsetYNew, True), Hex(0x677CB5, 6), 30) Then RemoveExtraTroopsQueue() ; RC Done
...
Func DragIfNeeded($Troop)
	...
	If IsDarkTroop($Troop) Then
		If _ColorCheck(_GetPixelColor(834, 403 + $g_iMidOffsetYNew, True), Hex(0xD3D3CB, 6), 5) Then $bCheckPixel = True ; RC Done
		...
		Next
	Else
		If _ColorCheck(_GetPixelColor(22, 403 + $g_iMidOffsetYNew, True), Hex(0xD3D3CB, 6), 5) Then $bCheckPixel = True ; RC Done
...
Func IsDarkSpell($Spell)
	...
	If $iIndex >= $ePSpell And $iIndex <= $eBaSpell Then Return True
	Return False
EndFunc   ;==>IsDarkSpell

Func IsSpellToBrew($sName)
	Local $iIndex = TroopIndexLookup($sName, "IsSpellToBrew")
	If $iIndex >= $eLSpell And $iIndex <= $eBaSpell Then Return True
	Return False
...
Func RemoveExtraTroops($toRemove)
	...
	If UBound($toRemove) > 0 Then ; If needed to remove troops
		...
		Click(Random(725, 825, 1), Random(540 + $g_iMidOffsetYNew, 565 + $g_iMidOffsetYNew, 1)) ; RC Done ; Click on Edit Army Button
		...
		For $i = 0 To 4 ; 5 x 400 = 2000ms
			If Not _ColorCheck(_GetPixelColor(806, 567 + $g_iMidOffsetYNew, True), Hex(0xCEEF76, 6), 25) Then ; RC Done ; If no 'Okay' button found in army tab to save changes
				If $i = 4 Then
		...
		If _Sleep(700) Then Return
		If Not $g_bRunState Then Return
		Click(Random(730, 815, 1), Random(558 + $g_iMidOffsetYNew, 589 + $g_iMidOffsetYNew, 1)) ; RC Done ; Click on 'Okay' button to save changes

		If _Sleep(1200) Then Return

		If Not _ColorCheck(_GetPixelColor(508, 428 + $g_iMidOffsetYNew, True), Hex(0xFFFFFF, 6), 30) Then ; RC Done ; If no 'Okay' button found to verify that we accept the changes
			SetLog("Cannot find/verify 'Okay #2' Button in Army tab", $COLOR_WARNING)
			ClickP($aAway, 2, 0, "#0346") ;Click Away
			Return False ; Exit function
		EndIf

		Click(Random(445, 580, 1), Random(402 + $g_iMidOffsetYNew, 455 + $g_iMidOffsetYNew, 1)) ; RC Done ; Click on 'Okay' button to Save changes... Last button
...
Func RemoveExtraTroopsQueue() ; Will remove All Extra troops in queue If there's a Low Opacity red color on them
	...
	Local Const $y = 186 + $g_iMidOffsetYNew, $yRemoveBtn = 200 + $g_iMidOffsetYNew, $xDecreaseRemoveBtn = 10 ; RC Done
...
Func IsQueueEmpty($sType = "Troops", $bSkipTabCheck = False, $removeExtraTroopsQueue = True)
	...
	If $removeExtraTroopsQueue Then
		If _ColorCheck(_GetPixelColor(230, 208 + $g_iMidOffsetYNew, True), Hex(0x677CB5, 6), 20) Then Return True ; RC Done ; If No troops were in Queue Return True
	Else
		If _ColorCheck(_GetPixelColor(820, 208 + $g_iMidOffsetYNew, True), Hex(0xD0D0C8, 6), 20) Then Return True ; RC Done ; check gray background at 1st training slot
	EndIf
...
Func GetSlotRemoveBtnPosition($iSlot, $bSpells = False)
	Local $iRemoveY = Not $bSpells ? 270 + $g_iMidOffsetYNew : 417 + $g_iMidOffsetYNew ; RC Done
...
Func GetSlotNumber($bSpells = False)
	Select
		Case $bSpells = False
			Local Const $Orders[21] = [$eBarb, $eArch, $eGiant, $eGobl, $eWall, $eBall, $eWiza, $eHeal, $eDrag, $ePekk, $eBabyD, $eMine, $eEDrag, _
		...
		Case $bSpells = True
			...
			Local Const $SpellsOrders[11] = [$eLSpell, $eHSpell, $eRSpell, $eJSpell, $eFSpell, $eCSpell, $ePSpell, $eESpell, $eHaSpell, $eSkSpell, $eBaSpell]
...
Func LeftSpace($bReturnAll = False)
	...
	Local $aRemainTrainSpace = GetOCRCurrent(48, 160 + $g_iMidOffsetYNew) ; RC Done
	If Not $g_bRunState Then Return
...
Func IsArmyWindow($bSetLog = False, $iTabNumber = 0)
	...
	Local $i = 0
	Local $_TabNumber[4][4] = [[114, 115 + $g_iMidOffsetYNew, 0xF8F8F8, 5], [284, 115 + $g_iMidOffsetYNew, 0xF8F8F8, 5], [505, 115 + $g_iMidOffsetYNew, 0xF8F8F8, 5], [702, 115 + $g_iMidOffsetYNew, 0xF8F8F8, 5]] ; RC Done ; Grey pixel on the tab name when is selected
...
Func CheckQueueTroops($bGetQuantity = True, $bSetLog = True, $x = 839)
	...
	Local $aSearchResult = SearchArmy($Dir, 18, 189 + $g_iMidOffsetYNew, $x, 253 + $g_iMidOffsetYNew) ; RC Done
	...
	If $bGetQuantity Then
		Local $aQuantities = GetQueueQuantity($aResult, $x - 65)
...
Func CheckQueueSpells($bGetQuantity = True, $bSetLog = True, $x = 839)
	...
	Local $aSearchResult = SearchArmy($DirSpellsQueued, 18, 189 + $g_iMidOffsetYNew, $x, 253 + $g_iMidOffsetYNew) ; RC Done
	ReDim $aResult[UBound($aSearchResult)]
...
Func GetQueueQuantity($aAvailableTroops, $xQueue = 775)
	...
	If IsArray($aAvailableTroops) Then
		...
		Local $x = $xQueue, $y = 191 + $g_iMidOffsetYNew ; RC Done
		_CaptureRegion2()
...
Func SearchArmy($sImageDir = "", $x = 0, $y = 0, $x1 = 0, $y1 = 0, $sArmyType = "", $bSkipReceivedTroopsCheck = False)
	...
	For $iCount = 0 To 10
		...
		If Not getReceivedTroops(162, 200 + $g_iMidOffsetYNew, $bSkipReceivedTroopsCheck) Then ; RC Done
			; Perform the search
	...
	If $sArmyType = "Troops" Then
		For $i = 0 To UBound($aResult) - 1
			$aResult[$i][3] = Number(getBarracksNewTroopQuantity(Slot($aResult[$i][1], "troop"), 196 + $g_iMidOffsetYNew)) ; RC Done ; coc-newarmy
		Next
	EndIf
	If $sArmyType = "Spells" Then
		For $i = 0 To UBound($aResult) - 1
			$aResult[$i][3] = Number(getBarracksNewTroopQuantity(Slot($aResult[$i][1], "spells"), 341 + $g_iMidOffsetYNew)) ; RC Done ; coc-newarmy
			;SetLog("$aResult: " & $aResult[$i][0] & "|" & $aResult[$i][1] & "|" & $aResult[$i][2] & "|" & $aResult[$i][3])
		Next
	EndIf
	If $sArmyType = "CCSpells" Then
		For $i = 0 To UBound($aResult) - 1
			$aResult[$i][3] = Number(getBarracksNewTroopQuantity(Slot($aResult[$i][1], "troop"), 498 + $g_iMidOffsetYNew)) ; RC Done ; coc-newarmy
		Next
	EndIf
	If $sArmyType = "Heroes" Then
		For $i = 0 To UBound($aResult) - 1
			If StringInStr($aResult[$i][0], "Kingqueued") Then
				$aResult[$i][3] = getRemainTHero(620, 414 + $g_iMidOffsetYNew) ; RC Done
			ElseIf StringInStr($aResult[$i][0], "Queenqueued") Then
				$aResult[$i][3] = getRemainTHero(695, 414 + $g_iMidOffsetYNew) ; RC Done
			ElseIf StringInStr($aResult[$i][0], "Wardenqueued") Then
				$aResult[$i][3] = getRemainTHero(775, 414 + $g_iMidOffsetYNew) ; RC Done
			Else
				$aResult[$i][3] = 0
			EndIf
		Next
...
Func TrainArmyNumber($Army)
	...
	Local $a_TrainArmy[3][4] = [[784, 368 + $g_iMidOffsetYNew, 0x6fb830, 10], [784, 485 + $g_iMidOffsetYNew, 0x72bb2f, 10], [784, 602 + $g_iMidOffsetYNew, 0x71ba2f, 10]] ; RC Done
...
Func DeleteQueued($sArmyTypeQueued, $iOffsetQueued = 802)
	...
	While Not IsQueueEmpty($sArmyTypeQueued, True, False)
		...
		If _Sleep(20) Then Return
		If Not $g_bRunState Then Return
		Click($iOffsetQueued + 24, 202 + $g_iMidOffsetYNew, 2, 0)
		$x += 1
...
Func MakingDonatedTroops($sType = "All")
	...
	If $areThereDonTroop > 0 Then
		...
		For $i = 0 To UBound($avDefaultTroopGroup, 1) - 1
			...
			If $avDefaultTroopGroup[$i][4] > 0 Then
				$RemainTrainSpace = GetOCRCurrent(48, 160 + $g_iMidOffsetYNew) ; RC Done
	...
	If $areThereDonSiegeMachine > 0 Then
		...
		Local $aCheckIsAvailableSiege[3][4] = [[58, 556 + $g_iMidOffsetYNew, 0x47717E, 10], [229, 556 + $g_iMidOffsetYNew, 0x47717E, 10], [400, 556 + $g_iMidOffsetYNew, 0x47717E, 10]] ;0->WallWracker,1->BattleBlimp,2->StoneSlammer
		...
		Local $sSiegeInfo = getArmyCapacityOnTrainTroops(57, 160 + $g_iMidOffsetYNew) ; RC Done ; OCR read Siege built and total
...
Func CheckIsFullQueuedAndNotFullArmy()
	...
	Local $CheckTroop[4] = [824, 243 + $g_iMidOffsetYNew, 0x949522, 20] ; RC Done ; the green check symbol [bottom right] at slot 0 troop
	If Not $g_bRunState Then Return

	If Not OpenTroopsTab(True, "CheckIsFullQueuedAndNotFullArmy()") Then Return

	Local $aArmyCamp = GetOCRCurrent(48, 160 + $g_iMidOffsetYNew) ; RC Done
	If UBound($aArmyCamp) = 3 And $aArmyCamp[2] < 0 Then
		If _ColorCheck(_GetPixelColor($CheckTroop[0], $CheckTroop[1], True), Hex($CheckTroop[2], 6), $CheckTroop[3]) Then
			...
			DeleteQueued("Troops")
			If _Sleep(500) Then Return
			$aArmyCamp = GetOCRCurrent(48, 160 + $g_iMidOffsetYNew) ; RC Done
...
Func CheckIsEmptyQueuedAndNotFullArmy()
	...
	Local $CheckTroop[4] = [825, 204 + $g_iMidOffsetYNew, 0xCFCFC8, 15] ; RC Done ; the gray background at slot 0 troop
	Local $CheckTroop1[4] = [390, 130 + $g_iMidOffsetYNew, 0x78BE2B, 15] ; RC Done ; the Green Arrow on Troop Training tab
	If Not $g_bRunState Then Return
...
Func CheckValuesCost($Troop = "Arch", $troopQuantity = 1, $DebugLogs = 0)
	...
	If $g_bDebugSetlogTrain Or $DebugLogs Then
		...
		$TempColorToCheck = _GetPixelColor(223, 594 + $g_iMidOffsetYNew, True) ; RC Done
	...
	If _ColorCheck(_GetPixelColor(223, 594 + $g_iMidOffsetYNew, True), Hex(0xE8E8E0, 6), 20) Then ; RC Done ; Gray background window color
		; Village without Dark Elixir
		$nElixirCurrent = getResourcesValueTrainPage(315, 594 + $g_iMidOffsetYNew) ; RC Done ; ELIXIR
	Else
		; Village with Elixir and Dark Elixir
		$nElixirCurrent = getResourcesValueTrainPage(230, 594 + $g_iMidOffsetYNew) ; RC Done ; ELIXIR
		$nDarkCurrent = getResourcesValueTrainPage(382, 594 + $g_iMidOffsetYNew) ; RC Done ; DARK ELIXIR
	EndIf
	...
	ElseIf $iTroopIndex >= $eLSpell And $iTroopIndex <= $eBaSpell Then
