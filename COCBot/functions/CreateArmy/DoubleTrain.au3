...
Func DoubleTrain($bQuickTrain = False)
	...
	Local $Step = 1
	While 1
		Local $TroopCamp = GetCurrentArmy(48, 160 + $g_iMidOffsetYNew) ; RC Done
	...
	Else
		...
		$Step = 1
		While 1
			Local $SpellCamp = GetCurrentArmy(43, 160 + $g_iMidOffsetYNew) ; RC Done
...
Func TrainFullQueue($bSpellOnly = False, $bSetlog = True)
	...
	Local $CampOCR = GetCurrentArmy($bSpellOnly ? 43 : 48, 160 + $g_iMidOffsetYNew) ; RC Done
...
;~ 	If Not IsArray($ArmyCamp) Then $ArmyCamp = GetCurrentArmy(48, 160+ $g_iMidOffsetYNew) ; RC Done
	...
	For $i = 0 To 10
		If _ColorCheck(_GetPixelColor(825 - $i * 70, 186 + $g_iMidOffsetYNew, True), Hex(0xD7AFA9, 6), 20) Then ; RC Done ; Pink background found
			$XQueueStart -= 70.5 * $i
			ExitLoop
		EndIf
	Next
	...
	If $ArmyCamp[0] < $ArmyCamp[1] * 2 Then
		...
		$ArmyCamp = GetCurrentArmy(48, 160 + $g_iMidOffsetYNew) ; RC Done
...
;~ 	If Not IsArray($ArmyCamp) Then $ArmyCamp = GetCurrentArmy(43, 160+ $g_iMidOffsetYNew) ; RC Done
	If $ArmyCamp[0] = $ArmyCamp[1] * 2 And ((ProfileSwitchAccountEnabled() And $g_abAccountNo[$g_iCurAccount] And $g_abDonateOnly[$g_iCurAccount]) Or $g_iCommandStop = 0) Then Return True ; bypass Donate account when full queue
	...
	If $ArmyCamp[0] < $ArmyCamp[1] * 2 Then
		...
		If _Sleep(1000) Then Return
		Local $NewSpellCamp = GetCurrentArmy(43, 160 + $g_iMidOffsetYNew) ; RC Done
...
Func DoubleTrainSiege($bDebug)
	;0 for Wall Wrecker, 1 for Battle Blimp, 2 for Stone Slammer
	Local $iSiege = $eSiegeWallWrecker
	If $g_iTotalTrainSpaceSiege < 1 Then Return ; train no siege
	If $g_aiArmyCompSiegeMachine[$eSiegeWallWrecker] > 0 And $g_aiArmyCompSiegeMachine[$eSiegeBattleBlimp] > 0 And $g_aiArmyCompSiegeMachine[$eSiegeStoneSlammer] > 0 Then ; train both types of siege
		If $bDebug Then SetLog("Army has both types of siege. Double train siege might cause unbalance.", $COLOR_DEBUG)
	Else
		If $g_aiArmyCompSiegeMachine[$eSiegeBattleBlimp] = 1 Then
			$iSiege = $eSiegeBattleBlimp
		ElseIf $g_aiArmyCompSiegeMachine[$eSiegeStoneSlammer] > 1 Then
			$iSiege = $eSiegeStoneSlammer
		EndIf

		If Not OpenSiegeMachinesTab(True, "DoubleTrainSiege()") Then Return
		If _Sleep(500) Then Return
		Local $checkPixel[4] = [58 + $iSiege * 171, 556 + $g_iMidOffsetYNew, 0x47717E, 10] ; RC Done ; WallW = 58, BlimpB = 58 + 171 = 229, StoneS = 58 + 342 = 400
