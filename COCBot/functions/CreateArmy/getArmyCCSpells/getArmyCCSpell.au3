...
Func getArmyCCSpells($bOpenArmyWindow = False, $bCloseArmyWindow = False, $bCheckWindow = False, $bSetLog = True, $bNeedCapture = True)
	...
	Local $sCCSpellDiamond = GetDiamondFromRect("450,451,610,549") ; RC Done ; New Cords Needed for reading 2 Spells.
		...
		For $i = 0 To UBound($aCurrentCCSpells, 1) - 1 ; Loop through found Spells
			...
			If StringInStr($aTempSpellArray[1], "|") Then
				...
				For $i = 1 To $aSpells[0]
					...
					$g_aiCurrentCCSpells[$iSpellIndex] += Number(getBarracksNewTroopQuantity(Slot($aSpellCoords[0], $aSpellCoords[1]), 498 + $g_iMidOffsetYNew)) ; RC Done ; Get The Quantity of the Spell, Slot() Does return the exact spot to read the Number from
				Next
			Else
