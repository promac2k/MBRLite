...
#pragma compile(ProductName, MultiBot Mini Gui)
#pragma compile(Out, MultiBot.run.MiniGui.exe) ; Required
...
Global $g_sBotTitle = "MultiBot Mini " & $g_sLiteVersion & " " ;~ Don't use any non file name supported characters like \ / : * ? " < > |
Global $g_hFrmBot = 0
...
Func UpdateBotTitle($sTitle = "MultiBot " & $g_sLiteVersion)
	$sTitle = StringReplace($sTitle, "MultiBot", "MultiBot Mini")
...
Func chkLiteVersion()
	$g_bChkFarmVersion = GUICtrlRead($g_hChkFarmVersion) = $GUI_CHECKED ? True : False
EndFunc   ;==>chkLiteVersion
...
Func GUIControl_WM_COMMAND($hWind, $iMsg, $wParam, $lParam)
	...
	Switch $nID
		...
		Case $g_hFrmBot_URL_PIC, $g_hFrmBot_URL_PIC2
			...
			OpenURL_Label("https://forum.MultiBot.run/")
		Case $g_hPicRegisterNow
			; Donate URL is not in text nor tooltip
			ShellExecute("https://forum.MultiBot.run/")
		Case $g_hBtnStop
			btnStop()
		Case $g_hBtnPause
			btnPause()
...
Func btnVillageStat($source = "")
	...
	If $g_iCurrentReport = $g_iVillageHourlyReport Then
		...
		If $g_iFirstRun = 1 Then GUICtrlSetState($g_hPicResultDETemp, $GUI_ENABLE + $GUI_SHOW)
	...
	ElseIf $g_iCurrentReport = $g_iBBReport Then
		...
		GUICtrlSetData($g_hLblVillageReportTemp, GetTranslatedFileIni("MBR GUI Design Bottom", "LblVillageReportTemp_04", "BB Report") & @CRLF & _
				GetTranslatedFileIni("MBR GUI Design Bottom", "LblVillageReportTemp_02", "will appear here") & @CRLF & _
				GetTranslatedFileIni("MBR GUI Design Bottom", "LblVillageReportTemp_03", "on first run."))
		GUICtrlSetData($g_hGrpVillage, GetTranslatedFileIni("MBR Main GUI", "Tab_04", -1) & ": " & $g_sProfileCurrentName)
	EndIf
...
Func chkPlayBBOnly()
	...
	If $g_bChkPlayBBOnly Then
		If GUICtrlRead($g_hChkFarmVersion) = $GUI_CHECKED Then ; Uncheck Farm
			_GUI_Value_STATE("UNCHECKED", $g_hChkFarmVersion)
			chkLiteVersion()
		EndIf
...
Func LaunchBotBackend($bNoGUI = True)
	...
	SetLog("Check if backend MultiBot process is already running...")
	While __TimerDiff($hTimer) < 5 * 60 * 1000

		If $g_WatchOnlyClientPID = Default And __TimerDiff($hTimer) > $g_iBotBackendFindTimeout Then
			$bCheck = False
			SetLog("MultiBot backend process not found, launching now...")
			SetDebugLog("MultiBot backend process launching: " & $cmd)
			$pid = Run($cmd, @ScriptDir)
			If $pid = 0 Then
				SetLog("Cannot launch MultiBot backend process", $COLOR_RED)
				Return 0
			EndIf
			If $g_iDebugSetlog Then
				SetDebugLog("MultiBot backend process launched, PID = " & $pid)
			Else
				SetLog("MultiBot backend process launched")
			EndIf
		...
		Else
			If $g_WatchOnlyClientPID <> Default And ProcessExists($g_WatchOnlyClientPID) = 0 Then
				SetDebugLog("MultiBot backend process not launched, PID = " & $g_WatchOnlyClientPID)
				$g_WatchOnlyClientPID = Default
			EndIf
		EndIf
...
Func ChkVerificationToken($Reset = False)
	Local Static $tokenwasverified[2] = [False, False]
	Local Static $FirstRun = True

	If $Reset Then $FirstRun = True

	If $tokenwasverified[0] = False And $FirstRun Then
		Local $sToken = $g_sTxtRegistrationToken
		If $Reset Then $sToken = GUICtrlRead($g_hTxtRegistrationToken)
		...
		If UBound($aToken) = 5 Then
			...
			If IsArray($Verify) Then
				If $Verify[0] == "Ok" Then
					SetLog("Free Token signing/verification completed!", $COLOR_SUCCESS)
