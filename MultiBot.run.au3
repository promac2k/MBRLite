...
#include "MultiBot.run.version.au3"
#pragma compile(ProductName, MultiBot)
#pragma compile(Out, MultiBot.run.exe) ; Required
...
Func UpdateBotTitle()
	Local $sTitle = "MultiBot Lite " & $g_sLiteVersion
...
Func InitializeMBR(ByRef $sAI, $bConfigRead)
	...
	Local $cmdLineHelp = GetTranslatedFileIni("MBR GUI Design - Loading", "Commandline_multiple_Bots", "By using the commandline (or a shortcut) you can start multiple Bots:\r\n" & _
			"     MultiBot.run.exe [ProfileName] [EmulatorName] [InstanceName]\r\n\r\n" & _
	...
	$sMsg = GetTranslatedFileIni("MBR GUI Design - Loading", "Msg_Android_instance_01", "MultiBot for %s is already running.\r\n\r\n", $sAI)
	...
	$sMsg = GetTranslatedFileIni("MBR GUI Design - Loading", "Msg_Android_instance_02", "MultiBot with Profile %s is already in use.\r\n\r\n", $g_sProfileCurrentName)
	...
	; Get mutex
	$g_hMutex_MyBot = CreateMutex("MultiBot.run")
	$g_bOnlyInstance = $g_hMutex_MyBot <> 0 ; And False
	SetDebugLog("MultiBot is " & ($g_bOnlyInstance ? "" : "not ") & "the only running instance")
...
Func FinalInitialization(Const $sAI)
	...
	If CheckPrerequisites(True) Then
		MBRFunc(True) ; start MultiBot.run.dll, after this point .net is initialized and threads popup all the time
		...
		Else
			...
			If $g_bIsClientSyncError = False And $g_bIsSearchLimit = False And ($g_bQuickAttack = False) Then
				...
				If IsSearchAttackEnabled() Then ; If attack scheduled has attack disabled now, stop wall upgrades, and attack.
					...
					If $g_bChkFarmVersion = False Then UpgradeWall()
					If _Sleep($DELAYRUNBOT3) Then Return
					If $g_bRestart = True Or $g_bChkPlayBBOnly = True Then ContinueLoop
	...
	While $g_bIsFullArmywithHeroesAndSpells = False
		...
		If $g_iActiveDonate And $g_bChkDonate Then
			...
			While $iReHere < 7
				...
				If $g_bRestart = True Or $g_bChkPlayBBOnly = True Then ExitLoop
		...
		If $g_bChkFarmVersion = False Then ReplayShare($g_bShareAttackEnableNow)
		...
		If $g_bChkFarmVersion = False Then AddIdleTime()
		checkMainScreen(False) ; required here due to many possible exits
		...
		If $g_bCanRequestCC = True And $g_bChkFarmVersion = False Then RequestCC()
		...
		If ($g_iAvailableAttacksBB > 0 Or $g_bChkBBStopAt3 = False) And $g_bChkBuilderAttack Then
			runBuilderBase()
		EndIf

		If $g_iCommandStop = -1 And $g_bChkFarmVersion = False Then ; Check if closing bot/emulator while training and not in halt mode
			SmartWait4Train()
			If Not $g_bRunState Then Return
			If $g_bRestart = True Or $g_bChkPlayBBOnly = True Then ExitLoop ; if smart wait activated, exit to runbot in case user adjusted GUI or left emulator/bot in bad state
		EndIf

		ClickP($aAway, 1, 0, "#0000") ;Click Away to prevent any pages on top ; Keep active
	WEnd
EndFunc   ;==>_Idle
...
Func _RunFunction($action)
	...
	Switch $action
		Case "Collect"
			If ($g_iCmbBoostBarracks = 0 Or $g_bFirstStart) And $g_bChkFarmVersion = False Then Collect()
			_Sleep($DELAYRUNBOT1)
		Case "CheckTombs"
			If ($g_iCmbBoostBarracks = 0 Or $g_bFirstStart) And $g_bChkFarmVersion = False Then CheckTombs()
			_Sleep($DELAYRUNBOT3)
		Case "CleanYard"
		...
		Case "NotifyReport"
			...
			_Sleep($DELAYRUNBOT3)
		Case "DonateCC"
			If $g_bChkFarmVersion = False Or (ProfileSwitchAccountEnabled() And Not $g_abDonateOnly[$g_iCurAccount]) Then
				If $g_iActiveDonate And $g_bChkDonate Then
		...
		Case "BoostEverything"
			BoostEverything()
		Case "LabCheck"
			;If $g_iCmbBoostBarracks = 0 Or $g_bFirstStart Then LabGuiDisplay()
			_Sleep($DELAYRUNBOT3)
		Case "RequestCC"
			If $g_bChkFarmVersion = False Then RequestCC()
			If _Sleep($DELAYRUNBOT1) = False Then checkMainScreen(False)
		Case "Laboratory"
			If ($g_iCmbBoostBarracks = 0 Or $g_bFirstStart) And $g_bChkFarmVersion = False Then Laboratory()
			If _Sleep($DELAYRUNBOT3) = False Then checkMainScreen(False)
		Case "UpgradeHeroes"
			If ($g_iCmbBoostBarracks = 0 Or $g_bFirstStart) And $g_bChkFarmVersion = False Then UpgradeHeroes()
			_Sleep($DELAYRUNBOT3)
		Case "UpgradeBuilding"
			If ($g_iCmbBoostBarracks = 0 Or $g_bFirstStart) And $g_bChkFarmVersion = False Then UpgradeBuilding()
			_Sleep($DELAYRUNBOT3)
			If ($g_iCmbBoostBarracks = 0 Or $g_bFirstStart) And $g_bChkFarmVersion = False Then AutoUpgrade()
			_Sleep($DELAYRUNBOT3)
		Case "BuilderBase"
			runBuilderBase()
			_Sleep($DELAYRUNBOT3)
		Case "CollectFreeMagicItems"
			If ($g_iCmbBoostBarracks = 0 Or $g_bFirstStart) And $g_bChkFarmVersion = False Then CollectFreeMagicItems()
		Case ""
...
Func FirstCheck()
	...
	If BotCommand() Then btnStop()
	; request troops will be the first routine to make , giving time to receive donated troops!
	RequestCC()
